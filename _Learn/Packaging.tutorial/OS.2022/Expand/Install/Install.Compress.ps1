...
Get-WindowsImage -ImagePath "D:\OS_10\Sources\install.wim" -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    dism /export-image /SourceImageFile:"D:\OS_10\Sources\install.wim" /SourceIndex:"$($_.ImageIndex)" /DestinationImageFile:"D:\OS_10\Sources\install.esd" /Compress:recovery /CheckIntegrity
    Write-Host "Compression completed`n" -ForegroundColor Green
}

Write-Host "`nVerify completion and delete old files"
if (Test-Path -Path "D:\OS_10\Sources\install.esd" -PathType leaf) {
