...
Get-WindowsImage -ImagePath "D:\ISOTemp\Sources\install.wim" -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    dism /export-image /SourceImageFile:"D:\ISOTemp\Sources\install.wim" /SourceIndex:"$($_.ImageIndex)" /DestinationImageFile:"D:\ISOTemp\Sources\install.esd" /Compress:recovery /CheckIntegrity
    Write-Host "Compression completed`n" -ForegroundColor Green
}

Write-Host "`nVerify completion and delete old files"
if (Test-Path -Path "D:\ISOTemp\Sources\install.esd" -PathType leaf) {
