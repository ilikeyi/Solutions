...
Get-WindowsImage -ImagePath "D:\ISOTemp\Sources\install.swm" -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    dism /export-image /SourceImageFile:"D:\ISOTemp\Sources\install.swm" /swmfile:"D:\ISOTemp\sources\install*.swm" /SourceIndex:"$($_.ImageIndex)" /DestinationImageFile:"D:\ISOTemp\Sources\install.wim" /Compress:"Max" /CheckIntegrity
    Write-Host "Export Complete`n" -ForegroundColor Green
}

Write-Host "`nVerify completion and delete old files"
if (Test-Path -Path "D:\ISOTemp\Sources\install.wim" -PathType leaf) {
    Get-ChildItem -Path "D:\ISOTemp\sources" -Recurse -include "*.swm" | ForEach-Object {
