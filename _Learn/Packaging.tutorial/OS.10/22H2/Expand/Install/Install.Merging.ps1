...
Get-WindowsImage -ImagePath "D:\OS_10\Sources\install.swm" -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    dism /export-image /SourceImageFile:"D:\OS_10\Sources\install.swm" /swmfile:"D:\OS_10\sources\install*.swm" /SourceIndex:"$($_.ImageIndex)" /DestinationImageFile:"D:\OS_10\Sources\install.wim" /Compress:"Max" /CheckIntegrity
...
if (Test-Path -Path "D:\OS_10\Sources\install.wim" -PathType leaf) {
    Get-ChildItem -Path "D:\OS_10\sources" -Recurse -include "*.swm" | ForEach-Object {
