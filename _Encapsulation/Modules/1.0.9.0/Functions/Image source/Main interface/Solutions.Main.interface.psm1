	...
	Function Image_Select_Refresh_Mount_Disk
	{
		...
		if ($MarkRAMDISK_Is_Check) {
			if ($MarkRAMDISK_Match_Done) {
				...
				$UI_Mask_Image_Mount_To_Tips.Text = "$($lang.AutoSelectRAMDISKFailed): $($Init_RAMDISK_Volume_Label)"
			}
		} else {
	...
	$GUIImageSource_Setting_RAMDISK_Exclude = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($GUIImageSource_Setting_RAMDISK_Exclude.Checked) {
				...
				Save_Dynamic -regkey "Solutions" -name "RAMDisk_Exclude" -value "False" -String
