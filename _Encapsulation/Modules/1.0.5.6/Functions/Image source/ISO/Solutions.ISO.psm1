...
Function ISO_Create_UI
{
	...
	$UI_Main_Event_Clear_Click = {
		...
		$Global:CreateSHA256ToSuffix = $False
	...
	Function ISO_Create_Refresh_Event_Status
	{
		...
		if ($UI_Main_Is_Create_ISO.Enabled) {
			if ($UI_Main_Is_Create_ISO.Checked) {
				...
				$GUIISOCreateSHA256ToSuffix.Enabled = $True
				...
				$GUIISOCreateSHA256ToSuffix.Enabled = $False
...
#		}
	...
	$GUIISOCreateSHA256ToSuffix = New-Object System.Windows.Forms.CheckBox -Property @{
		Height         = 35
		Width          = 455
		Padding        = "38,0,0,0"
		Text           = $lang.CreateSHA256ToSuffix
		add_Click      = {
			$UI_Main_Error.Text = ""
			$UI_Main_Error_Icon.Image = $null
		}
	...
	$GUIISOCreateSHA256_Warp = New-Object system.Windows.Forms.Label -Property @{
		Height         = 15
		Width          = 455
	}
		...
		if ($UI_Main_Is_Create_ISO.Checked) {
			...
			if ($FlagCheckISOName) {
				...
				if ($FlagCreateISOTest) {
					...
					$Global:CreateSHA256ToSuffix = $False
					if ($GUIISOCreateSHA256ToSuffix.Enabled) {
						if ($GUIISOCreateSHA256ToSuffix.Checked) {
							$Global:CreateSHA256ToSuffix = $True
						}
					}
	...
	$UI_Main_Adv_Expand_Panel.controls.AddRange((
		...
		$GUIISOBypassTPM,
		$GUIISOCreateSHA256,
		$GUIISOCreateSHA256ToSuffix,
		$GUIISOCreateSHA256_Warp,
		$GUIISOCreateASC,
	...
	if ($Autopilot) {
		...
		#>
			...
			<#
				...
				.处理：创建 SHA256
			#>
			if ($Autopilot.After.CRCSHA.SHA256.IsAllow) {
				...
				<#
					.Processing: Add SHA-256 to the filename extension
					.将 SHA-256 添加到文件名后缀
				#>
				if ($Autopilot.After.CRCSHA.SHA256.Rename) {
					$GUIISOCreateSHA256ToSuffix.Checked = $true
				} else {
					$GUIISOCreateSHA256ToSuffix.Checked = $False
				}
		...
		if (Test-Path -Path $Global:ISOSaveToFolder -PathType Container) {
			...
			if ($Global:CreateSHA256) {
				...
				<#
					.删除旧文件 .sha256
				#>
				$CustomNewSha256Fullname = "$($RenameNewISOFile).sha256"
				Remove-Item -path $CustomNewSha256Fullname -Force -ErrorAction SilentlyContinue

				<#
					.开始生成
				#>
				$calchash = (Get-FileHash -Path $RenameNewISOFile -Algorithm SHA256)
				"$($calchash.Hash)  $($Global:ISOSaveToFileName)" | Out-File -FilePath $CustomNewSha256Fullname -Encoding ASCII -ErrorAction SilentlyContinue | Out-Null

				if (Test-Path -Path $CustomNewSha256Fullname -PathType Leaf) {
					Write-Host " $($lang.Done) " -BackgroundColor DarkGreen -ForegroundColor White
				} else {
					Write-Host "$($lang.FailedCreateFile): $($RenameNewISOFile).sha256" -ForegroundColor Red
				}

				Write-Host "`n  $($lang.CreateSHA256ToSuffix)" -ForegroundColor Yellow
				Write-Host "  $('-' * 80)"
				write-host "  " -NoNewline 
				write-host " $($lang.Rename) " -NoNewline -BackgroundColor White -ForegroundColor Black
				if ($Global:CreateSHA256ToSuffix) {
					...
					if (Test-Path -Path $RenameNewISOFile -PathType Leaf) {
						...
						if (Test-Path -Path $CustomSHA256SUffix -PathType Leaf) {
							...
							Write-Host "     - $($CustomNewSha256Fullname)" -ForegroundColor Green
							Write-Host "     $('.' * 77)"
							Remove-Item -path $CustomNewSha256Fullname -Force -ErrorAction SilentlyContinue
							if (Test-Path -Path $CustomNewSha256Fullname -PathType Leaf) {
								Write-Host "     $($lang.Failed) " -ForegroundColor Red
				...
				if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
					if (Test-Path -Path $RenameNewISOFile -PathType Leaf) {
						...
						if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
							Write-Host "`n  $($lang.Command)" -ForegroundColor Yellow
							Write-Host "  $('-' * 80)"
							Write-Host "  Start-Process -FilePath '$($Verify_Install_Path)' -argument '$($arguments)'" -ForegroundColor Green
							Write-Host "  $('-' * 80)`n"
						}
