...
Function Language_Extract_UI
{
	...
	Function Extract_Language_Check_Customize
	{
		...
		$UI_Main_Extract_Not_ISO_Result.Text = ""
			...
			$UI_Main_Extract_Rule_Select_Sourcest.Controls | ForEach-Object {
				if ($_ -is [System.Windows.Forms.RadioButton]) {
					if ($_.Checked) {
						...
						Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)SelectGUID" -value $_.Tag
					}
				}
			}
		...
		if ($SelectLanguage) {
			...
			if ($Script:TempSelectLXPsLanguage.Count -gt 0) {
				...
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Add_List" -value $Script:TempSelectLXPsLanguage -Type "MultiString"
			} else {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.png")
		...
		if ($UI_Main_Dont_Use_Rules.Checked) {
			...
			if ($FolderName) {
				...
				ForEach ($item in $Global:Image_Rule) {
					iif ($Global:SMExt -contains $item.Main.Suffix) {
	...
	Function Language_Extract_Done_Eject_ISO_File
	{
		<#
			.检查是否选择规则命名
		#>
		$MarkCheckedRuleNaming = $False
		$LanguageSearchRuleSelected = ""

		$UI_Main_Extract_Rule_Select_Sourcest.Controls | ForEach-Object {
			if ($_ -is [System.Windows.Forms.RadioButton]) {
				if ($_.Checked) {
					$MarkCheckedRuleNaming = $True
					$LanguageSearchRuleSelected = $_.Tag
				}
			}
		}

		if ($MarkCheckedRuleNaming) {
			Language_Extract_Done_Eject_ISO_File_Force_Eject -GUID $LanguageSearchRuleSelected
		} else {
			$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.png")
			$UI_Main_Error.Text = "$($lang.SelectFromError): $($lang.NoChoose) ( $($lang.RulePre) )"
		}
	...
	Function Language_Extract_Done_Eject_ISO_File_Force_Eject
	{
		...
		if ($InBox_Apps_Rule_Select_Single.count -gt 0) {
			...
			$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.Alternative): $($NewInBoxAppsFolder)`n"
			$UI_Main_Extract_Not_ISO_Result.Text += "$('.' * 80)`n"
				...
				ForEach ($item in $InBox_Apps_Rule_Select_Single.Autopilot.Prerequisite.$NewInBoxAppsFolder.ISO.Language) {
					if ($item.Count -gt 0) {
						foreach ($ItemAlFile in $item) {
							$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.FileName): $($ItemAlFile)`n"
							...
							Get-Volume | Where-Object DriveType -eq 'CD-ROM' | ForEach-Object {
								Get-DiskImage -DevicePath $_.Path.trimend('\') -ErrorAction SilentlyContinue | ForEach-Object {
									...
									if ($NewFileName -eq $ItemAlFile) {
										$UI_Main_Extract_Not_ISO_Result.Text += "    $($lang.UpdateAvailable), $($lang.FileName): $($_.ImagePath)`n"
										$NewDriveLetter = (Get-DiskImage -ImagePath $_.ImagePath | Get-Volume).DriveLetter + ":\"
										$UI_Main_Extract_Not_ISO_Result.Text += "    $($lang.Select_Path): $($NewDriveLetter)`n"

										DisMount-DiskImage -ImagePath $_.ImagePath

										if (Test-Path -Path $NewDriveLetter -PathType Container) {
											$IsEjectNewTasks = "Failed"
										} else {
											$IsEjectNewTasks = "Yes"
										}
									}
							...
							$UI_Main_Extract_Not_ISO_Result.Text += "    $($lang.Eject): "
							switch ($IsEjectNewTasks) {
								"Yes" {
									$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.Done)"
								}
								"Failed" {
									$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.Failed)"
								}
								Default {
									$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.ImageCodenameNo)"
								}
							}

							$UI_Main_Extract_Not_ISO_Result.Text += "`n`n"
						}
					} else {
						$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.NoWork)`n"
					}
				}
			} else {
				$UI_Main_Extract_Not_ISO_Result.Text += "$($lang.NoWork)`n"
			...
			$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.png")
			$UI_Main_Error.Text = "$($lang.SelectFromError): $($lang.Export_Lang_Eject_ISO)"
		}
	}
	...
	$UI_Main_Extract_Rule_To_Open_Folder = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			if ([string]::IsNullOrEmpty($UI_Main_Extract_Rule_To_Result.Text)) {
				$UI_Main_View_Return_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.png")
				$UI_Main_View_Return_Error.Text = "$($lang.OpenFolder), $($lang.Inoperable)"
			} else {
				if (Test-Path -Path $UI_Main_Extract_Rule_To_Result.Text -PathType Container) {
					Start-Process $UI_Main_Extract_Rule_To_Result.Text

					$UI_Main_View_Return_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Success.png")
	...
	<#
		.提取完成后弹出已挂载的 ISO, 规则, 结果
	#>
	$UI_Main_Extract_Not_ISO = New-Object system.Windows.Forms.Label -Property @{
		Height         = 30
		Width          = 530
		margin         = "0,35,0,0"
		Text           = $lang.Export_Lang_Eject_ISO
	}
	$UI_Main_Extract_Not_ISO_Result = New-Object System.Windows.Forms.RichTextBox -Property @{
		Height         = 605
		Width          = 510
		margin         = "20,0,0,0"
		BorderStyle    = 0
	...
	$UI_Main_View_Return_Hide_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,635"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_View_Return_Hide = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,638"
		Text           = $lang.Hide
	...
	<#
		.提取完成后弹出已挂载的 ISO, 规则
	#>
	$UI_Main_Extract_Done_Eject_ISO = New-Object System.Windows.Forms.CheckBox -Property @{
		Height         = 40
		Width          = 530
		Padding        = "18,0,0,0"
		Text           = $lang.Export_Lang_Eject_ISO
	...
	$UI_Main_Extract_Done_Eject_ISO_Tips = New-Object system.Windows.Forms.Label -Property @{
		Height         = 40
		Width          = 530
		Padding        = "37,0,0,0"
		Margin         = "0,0,0,5"
	...
	<#
		.搜索或匹配
	#>
	$UI_Main_Setting_ISO_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,15"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_Extract_Search_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,55"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		...
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Extract_Search_Click
	}
	$UI_Main_Extract_Search = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 210
		Location       = "646,59"
	...
	$UI_Main_Extract_Import_Click = {
		if (Extract_Language_Check_Customize -RuleNaming -SelectLanguage -SelectWIM -AddAndDel -SavePath -FolderName) {
			...
			if ($UI_Main_Extract_Done_Eject_ISO.Checked) {
				Language_Extract_Done_Eject_ISO_File
			}
		}
	}
	$UI_Main_Extract_Import_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,95"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Import.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Extract_Import_Click
	}
	$UI_Main_Extract_Import = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 210
		Location       = "646,99"
		Text           = $lang.Import
	...
	$UI_Main.controls.AddRange((
		...
		$UI_Main_Menu,
		$UI_Main_Setting_ISO_ICO,
		$UI_Main_Setting_ISO,
		$UI_Main_Extract_Search_ICO,
		$UI_Main_Extract_Search,
		$UI_Main_Extract_Import_ICO,
	...
	$UI_Main_Menu.controls.AddRange((
		...
		$UI_Main_Extract_Done_Eject_ISO,
		$UI_Main_Extract_Done_Eject_ISO_Tips,
	...
	$UI_Main_View_Return.controls.AddRange((
		...
		$UI_Main_View_Return_Hide_ICO,
		$UI_Main_View_Return_Hide
	))
	$UI_Main_View_Return_Menu.controls.AddRange((
		...
		<#
			.提取完成后弹出已挂载的 ISO, 规则
		#>
		$UI_Main_Extract_Not_ISO,
		$UI_Main_Extract_Not_ISO_Result,
	...
	ForEach ($item in $SearchFolderRule) {
		...
		if ($item -eq $(Get_MainMasterFolder)) {
	...
	if (-not (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Add_List" -ErrorAction SilentlyContinue)) {
		...
		if ($DeployinboxGetSourcesOnly.Count -gt 0) {
			...
			Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Add_List" -value $DeployinboxGetSourcesOnly -Type "MultiString"
		} else {
			Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -name "$(Get_GPS_Location)_Add_List" -value "" -Type "MultiString"
		}
	}

	$GetSelectLXPsLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)_Add_List" -ErrorAction SilentlyContinue
...
#					$CheckBox.Checked = $True
	...
	<#
		.选择全局唯一规则 GUID
	#>
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)SelectGUID" -ErrorAction SilentlyContinue) {
		$GetDefaultSelectLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Language" -Name "$(Get_GPS_Location)SelectGUID" -ErrorAction SilentlyContinue
	} else {
	...
	if ($Autopilot) {
		...
		Write-Host " $($lang.LanguageExtract) " -NoNewline -BackgroundColor White -ForegroundColor Black
