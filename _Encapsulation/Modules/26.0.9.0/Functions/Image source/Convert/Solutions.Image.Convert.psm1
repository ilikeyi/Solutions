...
Function Image_Convert_UI
{
	...
	Function Autopilot_Image_Convert_Save
	{
		...
		if ($GUIImageConvertToESD.Checked) {
			...
			$Global:Queue_Convert_Tasks = @{
				Name        = "ESDtoWIM"
				Compression = $UI_Main_Convert_Type_Select.SelectedItem.CompressionType
	...
	Function Refresh_CompressionType
	{
		...
		switch ($Type) {
			"Max" {
				$UI_Main_Convert_Type_Select.SelectedIndex = $UI_Main_Convert_Type_Select.FindString($lang.CompressionType_Max)
			}
			"Fast" {
				$UI_Main_Convert_Type_Select.SelectedIndex = $UI_Main_Convert_Type_Select.FindString($lang.CompressionType_Fast)
			}
			"None" {
				$UI_Main_Convert_Type_Select.SelectedIndex = $UI_Main_Convert_Type_Select.FindString($lang.CompressionType_None)
			}
		}
	}
...
#		add_Click       = { Image_Convert_Refresh }
	...
	$UI_Main_Convert_Type_Select = New-Object system.Windows.Forms.ComboBox -Property @{
		Height         = 30
		Width          = 220
		Location       = "70,600"
		Text           = ""
	...
	$UI_Main_Convert_Type_Select.BindingContext = New-Object System.Windows.Forms.BindingContext
	$UI_Main_Convert_Type_Select.Datasource = $CompressionType
	$UI_Main_Convert_Type_Select.ValueMember = "CompressionType"
	$UI_Main_Convert_Type_Select.DisplayMember = "Lang"
...
Function Image_Convert_Process
{
	...
	Write-Host "  $('-' * 80)"
	$Host.UI.RawUI.WindowTitle = "$($Global:Author)'s Solutions | $($lang.Convert_Only), $($lang.Conver_Merged), $($lang.Conver_Split_To_Swm)"
	...
	switch ($Global:Queue_Convert_Tasks.Name)
	{
		...
		'ESDtoWIM'
		{
			...
			if ($Global:Queue_Convert_Tasks.Backup) {
				$SaveFileToEsd = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)\install.esd"
				...
				$SaveFileToEsdTemp = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)"
			...
			if ($Global:Developers_Mode) {
				Write-Host "`n  $($lang.Developers_Mode_Location): 6" -ForegroundColor Green
			}
			...
			if (Test-Path -Path $Install_wim -PathType leaf) {
				...
				if ($Global:Queue_Convert_Tasks.Split.IsSplit) {
					...
					if (Test-Path -Path $Install_wim -PathType leaf) {
						if ($Global:Developers_Mode) {
							Write-Host "`n  $($lang.Developers_Mode_Location): 7" -ForegroundColor Green
						}
		...
		'WIMtoESD'
		{
			...
			if ($Global:Queue_Convert_Tasks.Backup) {
				$SaveFileToWim = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)\install.wim"
				...
				$SaveFileToWimTemp = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)"
			...
			if ($Global:Developers_Mode) {
				Write-Host "`n  $($lang.Developers_Mode_Location): 2" -ForegroundColor Green
			}
		...
		'SplitWim'
		{
			...
			if ($Global:Queue_Convert_Tasks.Split.IsSplit) {
				...
				if (Test-Path -Path $Install_wim -PathType leaf) {
					if ($Global:Developers_Mode) {
						Write-Host "`n  $($lang.Developers_Mode_Location): j99" -ForegroundColor Green
					}
		...
		'MergedSWM'
		{
			...
			$SaveFileToSwm = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)\install*.swm"
			$SaveFileToSwmMatch = Join-Path -Path $Global:Image_source -ChildPath "Sources\*.swm"
			$SaveBackToGuid = Join-Path -Path $Global:MainMasterFolder -ChildPath "_Backup\Install.wim\$($RandomGuid)"
