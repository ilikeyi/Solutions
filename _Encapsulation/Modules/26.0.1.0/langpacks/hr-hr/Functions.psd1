ConvertFrom-StringData -StringData @'
	# hr-HR
	# Croatian (Croatia)

	SpecialFunction                 = PowerShell funkcije
	Functions_Before                = Prije trčanja
	Functions_Rear                  = Nakon završetka
	Functions_Running_Order         = Pokreni niz nakon dodjele
	Functions_Duplicate             = Nemojte dodavati ako postoje duplikati
	Functions_AutoSelect            = Automatski provjeravajte nove stavke nakon dodavanja
	Function_Unrestricted           = Neograničen

	Other_Tasks_CTD                 = Stvorite testni direktorij, glavni put nakon izvanmrežnog montiranja
	Other_Tasks_Clear               = Očisti sve generirano
	Other_Tasks_Empty               = Prazan zadatak
	Other_Tasks_Pause               = Pauza
	Other_Tasks_RAMDISK             = Pretražite sve jedinice lokalnog diska pomoću RADMDISK-a i zatim ih formatirajte
	Other_Tasks_RAMDISK_AR          = Pretražite sve jedinice lokalnog diska pomoću RADMDISK-a, a zatim ih formatirajte + automatski popravite
	Other_Tasks_REFS                = WinPE dodaje REFS instalacijsku podršku
	Other_Tasks_TPM                 = Zaobiđite provjeru instalacije TPM-a
'@