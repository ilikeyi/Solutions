		...
		If ([String]::IsNullOrEmpty($GetImportFileName)) {
			...
			if (Test-Path -Path $GetImportFileName -PathType leaf) {
				...
				if ($RunRule) {
					...
					Write-Host "  " -NoNewline
					Write-Host " $($lang.Running) " -NoNewline -BackgroundColor White -ForegroundColor Black
					if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($RuleName)" -Name "NoWait" -ErrorAction SilentlyContinue) {
						switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\API\Custom\$($RuleName)" -Name "NoWait" -ErrorAction SilentlyContinue) {
							"True" {
