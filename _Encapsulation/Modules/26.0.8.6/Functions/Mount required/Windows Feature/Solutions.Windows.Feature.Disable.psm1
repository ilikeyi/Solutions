Function Feature_Disable_UI
{
	...
	Function Refres_Event_Tasks_Feature_Disable
	{
	...
	Function Refresh_Wait_Add_Disable_List
	{
		$UI_Main_Error.Text = ""
		$UI_Main_Error_Icon.Image = $null
	...
	Function Disable_Select_Refresh_List
	{
		$UI_Main_Search_Sift_Custon.BackColor = "#FFFFFF"
		...
		<#
			.判断是否空白
		#>
		if ([string]::IsNullOrEmpty($UI_Main_Search_Sift_Custon.Text)) {
			...
			<#
				...
				.判断: 2. 前缀不能带空格
			#>
			if ($UI_Main_Search_Sift_Custon.Text -match '^\s') {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.ico")
				$UI_Main_Error.Text = "$($lang.SelectFromError): $($lang.ISO9660TipsErrorSpace)"
				$UI_Main_Search_Sift_Custon.BackColor = "LightPink"
				return
			}
			...
			<#
				...
				.判断: 4. 后缀不能带多空格
			#>
			if ($UI_Main_Search_Sift_Custon.Text -match '\s{2,}$') {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.ico")
				$UI_Main_Error.Text = "$($lang.SelectFromError): $($lang.ISO9660TipsErrorSpace)"
				$UI_Main_Search_Sift_Custon.BackColor = "LightPink"
				return
			...
			<#
				...
				.判断: 6, 不能包含: \\ / : * ? "" < > |
			#>
			if ($UI_Main_Search_Sift_Custon.Text -match '[~#$@!%&*{}<>?/|+"]') {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.ico")
			...
			<#
				...
				.判断: 7. 不能大于 260 字符
			#>
			if ($UI_Main_Search_Sift_Custon.Text.length -gt 260) {
				$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Error.ico")
			...
			ForEach ($WildCard in $Script:Init_Feature_All_Enabled) {
				...
				if ($NewFileName -like "*$($UI_Main_Search_Sift_Custon.Text)*") {
	...
	$UI_Main           = New-Object system.Windows.Forms.Form -Property @{
		...
		Height         = 720
		Width          = 928
		Text           = "$($lang.WindowsFeature): $($lang.Disable)"
	...
	<#
		.搜索或匹配
	#>
	$UI_Main_Search_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,15"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		...
		add_Click      = { Disable_Select_Refresh_List }
	}
	$UI_Main_Search    = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 210
		Location       = "646,17"
	...
	$UI_Main_Search_Sift = New-Object system.Windows.Forms.Label -Property @{
		Height         = 30
		Width          = 240
		Location       = "625,60"
		Text           = $lang.LanguageExtractRuleFilter
	}
	$UI_Main_Search_Sift_Custon = New-Object System.Windows.Forms.TextBox -Property @{
		Height         = 30
		Width          = 250
		Location       = "645,90"
		Text           = ""
	...
	$UI_Main_Save_Click = {
		<#
			.初始化临时保存的选择
		#>
		$Temp_Queue_Is_Feature_Disable_Select = @()
		$UI_Main_Wait_Add.Controls | ForEach-Object {
			if ($_ -is [System.Windows.Forms.CheckBox]) {
				if ($_.Enabled) {
					if ($_.Checked) {
						$Temp_Queue_Is_Feature_Disable_Select += $_.Tag
					}
				}
			}
		}
	...
	Get-WindowsOptionalFeature -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Get.log" -Path $test_mount_folder_Current -ErrorAction SilentlyContinue | ForEach-Object {
...
<#
	.Windows 功能: 处理禁用项
#>
Function Feature_Disable_Process
{
	$test_mount_folder_Current = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master).$($Global:Primary_Key_Image.MasterSuffix)\$($Global:Primary_Key_Image.ImageFileName).$($Global:Primary_Key_Image.Suffix)\Mount"
		...
		ForEach ($item in $Temp_Queue_Is_Feature_Disable_Custom_Select) {
			...
			try {
				if (Test-Path -Path $test_mount_folder_Current_Windows -PathType Container) {
					...
					Write-Host " $($lang.Disable) " -NoNewline -BackgroundColor White -ForegroundColor Black
