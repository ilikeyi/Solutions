Function Functions_Before_UI
{
	...
	Function Autopilot_Functions_Before_UI_Save
	{
		...
		if ($TempSelectFunctionQueue.Count -gt 0) {
			New-Variable -Scope global -Name "Queue_Functions_Before_Select_$($Global:Primary_Key_Image.Uid)" -Value $TempSelectFunctionQueue -Force
	...
	Function Refres_Event_Tasks_Functions_Before
	{
		...
		$Temp_Assign_Task_Select = (Get-Variable -Scope global -Name "Queue_Functions_Before_Select_$($Global:Primary_Key_Image.Uid)" -ErrorAction SilentlyContinue).Value
	...
	$UI_Main_Event_Clear_Click = {
		New-Variable -Scope global -Name "Queue_Functions_Before_Select_$($Global:Primary_Key_Image.Uid)" -Value @() -Force
	...
	$UI_Main_Functions_Duplicate = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Functions_Duplicate.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -name "$(Get_GPS_Location)_Is_Check_Duplicate" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -name "$(Get_GPS_Location)_Is_Check_Duplicate" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -Name "$(Get_GPS_Location)_Is_Check_Duplicate" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -Name "$(Get_GPS_Location)_Is_Check_Duplicate" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Functions_AutoSelect = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Functions_AutoSelect.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -name "$(Get_GPS_Location)_Is_Check_Auto_Select" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -name "$(Get_GPS_Location)_Is_Check_Auto_Select" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -Name "$(Get_GPS_Location)_Is_Check_Auto_Select" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Functions" -Name "$(Get_GPS_Location)_Is_Check_Auto_Select" -ErrorAction SilentlyContinue) {
			"True" {
