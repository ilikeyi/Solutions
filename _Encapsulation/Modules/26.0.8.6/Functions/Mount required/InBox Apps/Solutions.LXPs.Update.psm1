	...
	Function Refresh_Add_InBox_Apps_New_Sources
	{
		...
		if (([string]::IsNullOrEmpty($Temp_Assign_Task_Select))) {
			...
			if ($Temp_Queue_Is_InBox_Apps_Update.Count -gt 0) {
				...
				if (-not (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)_SelectLXPsLanguage" -ErrorAction SilentlyContinue)) {
					...
					if ($DeployinboxGetSourcesOnly.count -gt 0) {
						Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)_SelectLXPsLanguage" -value $DeployinboxGetSourcesOnly -Type "MultiString"
					} else {
						Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)_SelectLXPsLanguage" -value "" -Type "MultiString"
					}
				}
				$GetSelectLXPsLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)_SelectLXPsLanguage" -ErrorAction SilentlyContinue
			}
