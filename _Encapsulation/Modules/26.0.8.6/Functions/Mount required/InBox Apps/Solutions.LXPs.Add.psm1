...
Function LXPs_Region_Add
{
		...
		if ($UI_Main_Is_Install_LXPs.Checked) {
			...
			$UI_Main_Select_LXPs.Controls | ForEach-Object {
				if ($_ -is [System.Windows.Forms.CheckBox]) {
					if ($_.Enabled) {
						if ($_.Checked) {
							$Temp_Select_Experience_Pack_Queue += [pscustomobject]@{
								Language = $_.Name
								Path     = $_.Tag
							}
						}
		...
		if (Test-Path -Path "$($NewPath)\LocalExperiencePack" -PathType Container) {
			Get-ChildItem "$($NewPath)\LocalExperiencePack" -directory -ErrorAction SilentlyContinue | ForEach-Object {
				if (Test-Path -Path "$($_.FullName)\LanguageExperiencePack.*.appx" -PathType Leaf) {
					$FlagCheckSelectStatus += [pscustomobject]@{
						Region = $_.BaseName
						File   = $_.FullName
					}
				}
	...
	$UI_Main_Mask_Report_OK_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,595"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Success.ico")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Mask_Report_OK_Click
	}
	$UI_Main_Mask_Report_OK = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,599"
		Text           = $lang.OK
	...
	$UI_Main_Mask_Report_Hide_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,635"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_Mask_Report_Hide = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,638"
		Text           = $lang.Hide
	...
	$UI_Main_Report_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,55"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Report.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Report_Click
	}
	$UI_Main_Report    = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 210
		Location       = "646,57"
		Text           = $lang.AdvAppsDetailed
	...
	$UI_Main_Mask_Report.controls.AddRange((
		...
		$UI_Main_Mask_Report_OK_ICO,
		$UI_Main_Mask_Report_OK,
		$UI_Main_Mask_Report_Hide_ICO,
				...
				if ($InitlUWPPrePakcage.count -gt 0) {
					...
					ForEach ($item in $InitlUWPPrePakcage) {
						if ($InitlUWPPrePakcageExclude -notContains $item) {
							...
							Write-Host "$($SNTasks)/$($InitlUWPPrePakcage.count)" -NoNewline -ForegroundColor Green
...
Function LXPs_Save_Report_Process
{
	...
	ForEach ($itemRegion in $Region) {
		...
		if (Test-Path -Path $TempNewFileFullPath -PathType Leaf) {
			...
			$zipFile.Entries | Where-Object { $_.Name -like 'AppxManifest.xml' } | ForEach-Object {
