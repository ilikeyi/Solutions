<#
	...
	.添加更新
#>
Function Cumulative_updates_Add_UI
{
	param
	(
		[array]$Autopilot
	...
	Function Autopilot_Cumulative_updates_Add_UI_Save
	{
		$UI_Main_Error.Text = ""
	...
	$UI_Main_Dont_Checke_Is_File = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Checke_Is_File.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add" -value "False"
			}

			Update_Add_Refresh_Sourcs
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Auto_select_Folder = New-Object system.Windows.Forms.Label -Property @{
		...
		$UI_Main_Dont_Checke_Is_RulePre = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RulePre.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add" -value "True"
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add" -value "False"
				}

				Update_Add_Refresh_Sourcs
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add" -ErrorAction SilentlyContinue) {
				"True" {
		...
		$UI_Main_Dont_Checke_Is_RuleMultistage = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RuleMultistage.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add" -value "True"
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add" -value "False"
				}

				Update_Add_Refresh_Sourcs
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add" -ErrorAction SilentlyContinue) {
				"True" {
		...
		$UI_Main_Dont_Checke_Is_RuleOther = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RuleOther.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add" -value "True"
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add" -value "False"
				}

				Update_Add_Refresh_Sourcs
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add" -ErrorAction SilentlyContinue) {
				"True" {
	...
	$UI_Main_Refresh_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,15"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\refresh.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Refresh_Click
	}
	$UI_Main_Refresh = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 210
		Location       = "646,17"
		Text           = $lang.Refresh
	...
	$UI_Main_Select_Folder_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,55"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\select.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
	...
	$UI_Main_Mask_Tips_Global_Do_Not = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Tips_Global_Do_Not.Checked) {
				Save_Dynamic -regkey "Solutions" -name "Tips_Warning_CU_Global" -value "True"
				$UI_Main_Mask_Tips_Do_Not.Enabled = $False
			} else {
				Save_Dynamic -regkey "Solutions" -name "Tips_Warning_CU_Global" -value "False"
	...
	$UI_Main_Mask_Tips_Do_Not = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Tips_Do_Not.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "Tips_Warning_CU" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "Tips_Warning_CU" -value "False"
			}
		}
	}

	$UI_Main_Mask_Tips_Hide_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,635"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_Mask_Tips_Hide = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,638"
		Text           = $lang.Hide
	...
	$UI_Main_Save_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		...
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Save_ICO_Click
	}
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "Tips_Warning_CU_Global" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "Tips_Warning_CU_Global" -ErrorAction SilentlyContinue) {
			"True" {
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "Tips_Warning_CU" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "Tips_Warning_CU" -ErrorAction SilentlyContinue) {
			"True" {
				$MarkShowNewTips = $False
...
Function Autopilot_Cumulative_updates_Add_UI_Import
{
	param
	(
		$Tasks
...
Function Update_Add_Process
{
	...
	if ($Temp_Assign_Task_Select.count -gt 0) {
		...
		Write-Host "  $('-' * 80)"
		ForEach ($item in $Temp_Assign_Task_Select) {
			Write-host "  $($lang.SearchOrder): " -NoNewline -ForegroundColor Yellow
			Write-host " 1. $($lang.MountedIndex), 2. $($lang.Wim_Edition), 3. $($lang.RuleOther)" -ForegroundColor Green
			...
			<#
				.搜索自定义目录 Custom
			#>
			Write-host "`n  $($lang.SearchOrder): " -ForegroundColor Yellow
			Write-Host "  $('-' * 80)"
			$NewCustomPathFull = Join-Path -Path $item -ChildPath "Custom"
			if (Test-Path -Path $NewCustomPathFull -PathType Container) {
				<#
					.条件 1: 搜索索引号
				#>
				[int]$SNTasks = "0"
				$SearchOrderInxde = Join-Path -Path $NewCustomPathFull -ChildPath "Index"

				write-host "  1. $($lang.MountedIndex)" -ForegroundColor Yellow
				Write-Host "     $($lang.Select_Path): " -NoNewline
				Write-Host $SearchOrderInxde -ForegroundColor Yellow
				Write-Host "  $('-' * 80)"
				#region Search Order 1
				if ($IsAllowMatch) {
					if (Test-Path -Path $SearchOrderInxde -PathType Container) {
						$CurrentMountIndex = Image_Get_Mount_Index

						Write-host "`n     $($lang.Search): $($lang.MountedIndexSelect): " -NoNewline
						Write-host $CurrentMountIndex -ForegroundColor Green
						$WaitMatchIndex = Join-Path -Path $SearchOrderInxde -ChildPath $CurrentMountIndex

						Write-Host "     $($lang.Select_Path): " -NoNewline
						Write-Host $WaitMatchIndex -ForegroundColor Yellow

						Write-Host "     " -NoNewline
						Write-Host " $($lang.MatchMode) " -NoNewline -BackgroundColor White -ForegroundColor Black
						if (Test-Path -Path $WaitMatchIndex -PathType Container) {
							$IsAllowMatch = $False
							Write-Host " $($lang.Prerequisite_satisfy) " -BackgroundColor DarkGreen -ForegroundColor White

							Write-Host
							$CountFileIndex = @()
							Get-ChildItem $WaitMatchIndex -Recurse -Include $Global:Search_KB_File_Type -ErrorAction SilentlyContinue | ForEach-Object {
								$CountFileIndex += $_.FullName
							}

							if ($CountFileIndex.count -gt 0) {
								foreach ($Newfilename in $CountFileIndex) {
									$SNTasks++
									Update_Add_Path_Process -Newpath $Newfilename -NewEvent "$($SNTasks)/$($CountFileIndex.Count)"
								}
							} else {
								write-host "  $($lang.NoWork)" -ForegroundColor Red
							}
				...
				#>
				[int]$SNTasks = "0"
				$SearchOrderEdition = Join-Path -Path $NewCustomPathFull -ChildPath "Edition"

				write-host "`n  2. $($lang.Wim_Edition)" -ForegroundColor Yellow
				Write-Host "     $($lang.Select_Path): " -NoNewline
				Write-Host $SearchOrderEdition -ForegroundColor Yellow
				Write-Host "  $('-' * 80)"
				#region Search Order 2
				if ($IsAllowMatch) {
					if (Test-Path -Path $SearchOrderEdition -PathType Container) {
						try {
							...
							$CurrentMountEdition = (Get-WindowsEdition -ScratchDirectory "$(Get_Mount_To_Temp)" -LogPath "$(Get_Mount_To_Logs)\Get.log" -Path $test_mount_folder_Current).Edition

							Write-host "     $($lang.Search): $($lang.Wim_Edition): " -NoNewline
							Write-host $CurrentMountEdition -ForegroundColor Green
							$WaitMatchEdition = Join-Path -Path $SearchOrderEdition -ChildPath $CurrentMountEdition

							Write-Host "     $($lang.Select_Path): " -NoNewline
							Write-Host $WaitMatchEdition -ForegroundColor Yellow

							Write-Host "     " -NoNewline
							Write-Host " $($lang.MatchMode) " -NoNewline -BackgroundColor White -ForegroundColor Black
							if (Test-Path -Path $WaitMatchEdition -PathType Container) {
								$IsAllowMatch = $False
								Write-Host " $($lang.Prerequisite_satisfy) " -BackgroundColor DarkGreen -ForegroundColor White

								Write-Host
								$CountFileEdition = @()
								Get-ChildItem $WaitMatchEdition -Recurse -Include $Global:Search_KB_File_Type -ErrorAction SilentlyContinue | ForEach-Object {
									$CountFileEdition += $_.FullName
								}

								if ($CountFileEdition.count -gt 0) {
									foreach ($Newfilename in $CountFileEdition) {
										$SNTasks++
										Update_Add_Path_Process -Newpath $Newfilename -NewEvent "$($SNTasks)/$($CountFileEdition.Count)"
									}
								} else {
									write-host "  $($lang.NoWork)" -ForegroundColor Red
								}
			...
			if ($IsAllowMatch) {
				...
				$CountFileOter = @()
					Get-ChildItem -Path $item -File -ErrorAction SilentlyContinue | ForEach-Object {
						if ($Search_KB_File_Type -contains $_.Extension) {
							$CountFileOter += $_.FullName
						}
					}
					Get-ChildItem -Path $item -Directory -Exclude $ExcludeCustom | ForEach-Object {
				...
				if ($CountFileOter.count -gt 0) {
					foreach ($Newfilename in $CountFileOter) {
						$SNTasks++
						Update_Add_Path_Process -Newpath $Newfilename -NewEvent "$($SNTasks)/$($CountFileOter.Count)"
					}
				} else {
					write-host "  $($lang.NoWork)" -ForegroundColor Red
				}
			} else {
				write-host "     $($lang.Prerequisite_Not_satisfied)`n" -ForegroundColor Red
			}
			#endregion
		}
	} else {
...
Function Update_Add_Path_Process
{
	param (
		$Newpath,
		$NewEvent
	...
	if (Test-Path -Path $Newpath -PathType Leaf) {
		...
		Write-Host $NewEvent -NoNewline -ForegroundColor Green
		...
		$test_mount_folder_Current = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master).$($Global:Primary_Key_Image.MasterSuffix)\$($Global:Primary_Key_Image.ImageFileName).$($Global:Primary_Key_Image.Suffix)\Mount"
		$CommandNew = "Add-WindowsPackage -ScratchDirectory ""$(Get_Mount_To_Temp)"" -LogPath ""$(Get_Mount_To_Logs)\Add.log"" -Path ""$($test_mount_folder_Current)"" -PackagePath ""$($Newpath)"" -ErrorAction SilentlyContinue | Out-Null"
		$CommandNewPrint = "Add-WindowsPackage -Path ""$($test_mount_folder_Current)"" -PackagePath ""$($Newpath)"""
