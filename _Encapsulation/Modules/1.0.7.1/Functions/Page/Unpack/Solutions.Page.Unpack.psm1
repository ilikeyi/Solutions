		...
		if ($Script:UnPackCreateASC) {
			...
			if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
				...
				if (Test-Path -Path $fullnewpathasc -PathType leaf) {
					...
					if ([string]::IsNullOrEmpty($Global:secure_password)) {
						Start-Process -FilePath $Verify_Install_Path -argument "--local-user ""$($GUIUnPackASCSign.Text)"" --output ""$($fullnewpathasc)"" --detach-sign ""$($fullnewpath)""" -Wait -WindowStyle Minimized
					} else {
						Start-Process -FilePath $Verify_Install_Path -argument "--pinentry-mode loopback --passphrase ""$($Global:secure_password)"" --local-user ""$($GUIUnPackASCSign.Text)"" --output ""$($fullnewpathasc)"" --detach-sign ""$($fullnewpath)""" -Wait -WindowStyle Minimized
					}
