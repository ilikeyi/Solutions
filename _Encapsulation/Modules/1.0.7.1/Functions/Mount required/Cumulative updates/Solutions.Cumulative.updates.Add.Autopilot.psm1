<#
	...
	.自动驾驶：添加更新
#>
Function Cumulative_updates_Add_UI_Autopilot
{
	<#
		.转换架构类型
	#>
	...
	#>
	$SearchFolderRule = @(
		Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\$($ArchitectureNew)\Add"
		"$(Get_MainMasterFolder)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Update\Add"
	)
	...
	Function Refres_Event_Tasks_Cumulative_updates_Add_UI_Autopilot
	{
		<#
			.全局
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$GetSelectVer = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue
		...
		<#
			.当前
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$GetSelectVer = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue
			$GetSelectVer = $GetSelectVer | Where-Object { -not ([string]::IsNullOrEmpty($_) -or [string]::IsNullOrWhiteSpace($_))} | Select-Object -Unique
	...
	$UI_Main_Create_New_Tempate_Click = {
		...
		Update_Add_Refresh_Sourcs_Autopilot
	}

	Function Update_Add_Refresh_Sourcs_Autopilot
	{
		$UI_Main_Error.Text = ""
		$UI_Main_Error_Icon.Image = $null
		...
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$Temp_Assign_Task_Select = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue
		}
		...
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$Temp_Assign_Task_Select = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue
		}
		...
		if ($SearchFolderRule.Count -gt 0) {
			...
			ForEach ($item in $SearchFolderRule) {
				...
				if (Test-Path -Path $item -PathType Container) {
					...
					$NewTipsText = "$($lang.SearchOrder): "
					$SearchOrderInxde = Join-Path -Path $item -ChildPath "Custom\Index"
					if (Test-Path -Path $SearchOrderInxde -PathType Container) {
						$NewTipsText += "$($lang.MountedIndex), $($lang.Prerequisite_satisfy)"
					} else {
						$NewTipsText += "$($lang.MountedIndex), $($lang.Inoperable)"
					}

					$SearchOrderEdition = Join-Path -Path $item -ChildPath "Custom\Edition"
					if (Test-Path -Path $SearchOrderEdition -PathType Container) {
						$NewTipsText += ", $($lang.Wim_Edition), $($lang.Prerequisite_satisfy)"
					} else {
						$NewTipsText += ", $($lang.Wim_Edition), $($lang.Inoperable)"
					}
					$AddSourcesSearchOrder_Wrap = New-Object system.Windows.Forms.Label -Property @{
						Height         = 30
						Width          = 520
						Padding        = "46,0,0,0"
						Text           = $NewTipsText
					...
					if ($UI_Main_Dont_Checke_Is_File.Checked) {
						...
						if((Get-ChildItem $item -Recurse -Include $Global:Search_KB_File_Type -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0) {
							...
							$UI_Main_Rule.controls.AddRange($AddSourcesSearchOrder_Wrap)
							$CheckBox.Enabled = $True
						}
					}
			...
			ForEach ($item in $Search_Folder_Multistage_Rule) {
				...
				if ($MarkIsFolderRule) {
					...
					$No_Find_Multistage_Rule_Create_New_Template = New-Object system.Windows.Forms.LinkLabel -Property @{
						...
						LinkBehavior    = "NeverUnderline"
						add_Click       = {
							Create_Template_UI -Auto -NewAdd
							Update_Add_Refresh_Sourcs_Autopilot
						}
					}
					...
					Get-ChildItem -Path $item -Directory -ErrorAction SilentlyContinue | Where-Object {
						...
						Update_Add_Refresh_Sources_New_Autopilot -Sources $_.FullName -NewMaster $Global:Primary_Key_Image.Master -ImageName $Global:Primary_Key_Image.ImageFileName
	...
	Function Update_Add_Refresh_Sources_New_Autopilot
	{
		param
		(
			$NewMaster,
		...
		if (Test-Path -Path $MarkNewFolder -PathType Container) {
			...
			$NewTipsText = "$($lang.SearchOrder): "
			$SearchOrderInxde = Join-Path -Path $MarkNewFolder -ChildPath "Custom\Index"
			if (Test-Path -Path $SearchOrderInxde -PathType Container) {
				$NewTipsText += "$($lang.MountedIndex), $($lang.Prerequisite_satisfy)"
			} else {
				$NewTipsText += "$($lang.MountedIndex), $($lang.Inoperable)"
			...
			$SearchOrderEdition = Join-Path -Path $MarkNewFolder -ChildPath "Custom\Edition"
			if (Test-Path -Path $SearchOrderEdition -PathType Container) {
				$NewTipsText += ", $($lang.Wim_Edition), $($lang.Prerequisite_satisfy)"
			} else {
				$NewTipsText += ", $($lang.Wim_Edition), $($lang.Inoperable)"
	...
	$UI_Main           = New-Object system.Windows.Forms.Form -Property @{
		...
		Height         = 720
		Width          = 928
		Text           = "$($lang.CUpdate): $($lang.AddTo) [ $($lang.Autopilot), $($lang.Event_Primary_Key): $($Global:Primary_Key_Image.Uid) ]"
	...
	$UI_Main_Dashboard_Event_Global_Clear = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		LinkBehavior   = "NeverUnderline"
		add_Click      = {
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -Force -ErrorAction SilentlyContinue | out-null

			Refres_Event_Tasks_Cumulative_updates_Add_UI_Autopilot
	...
	$UI_Main_Dashboard_Event_Current_Clear = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		LinkBehavior   = "NeverUnderline"
		add_Click      = {
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -Force -ErrorAction SilentlyContinue | out-null
	...
	$UI_Main_Dont_Checke_Is_File = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Checke_Is_File.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -value "True" -String
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -value "False" -String
			}

			Update_Add_Refresh_Sourcs_Autopilot
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Auto_select_Folder = New-Object system.Windows.Forms.Label -Property @{
		...
		$UI_Main_Dont_Checke_Is_RulePre = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RulePre.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -value "True" -String
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -value "False" -String
				}

				Update_Add_Refresh_Sourcs_Autopilot
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -ErrorAction SilentlyContinue) {
				"True" {
		...
		$UI_Main_Dont_Checke_Is_RuleMultistage = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RuleMultistage.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add_Autopilot" -value "True" -String
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add_Autopilot" -value "False" -String
				}

				Update_Add_Refresh_Sourcs_Autopilot
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add_Autopilot" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleMultistage_Add_Autopilot" -ErrorAction SilentlyContinue) {
				"True" {
		...
		$UI_Main_Dont_Checke_Is_RuleOther = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RuleOther.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -value "True" -String
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -value "False" -String
				}

				Update_Add_Refresh_Sourcs_Autopilot
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -ErrorAction SilentlyContinue) {
				"True" {
	...
	$UI_Main_Mask_Tips_Global_Do_Not = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Tips_Global_Do_Not.Checked) {
				Save_Dynamic -regkey "Solutions" -name "Tips_Warning_CU_Global" -value "True" -String
				$UI_Main_Mask_Tips_Do_Not.Enabled = $False
			} else {
				Save_Dynamic -regkey "Solutions" -name "Tips_Warning_CU_Global" -value "False" -String
	...
	$UI_Main_Mask_Tips_Do_Not = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Tips_Do_Not.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "Tips_Warning_CU" -value "True" -String
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "Tips_Warning_CU" -value "False" -String
			}
		}
	}
	...
	$UI_Main_Event_Sync_To_Global = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Event_Sync_To_Global.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -value "True" -String
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -value "False" -String
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Save      = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ($Temp_Queue_Select_Item.Count -gt 0) {
				if ($UI_Main_Event_Sync_To_Global.Checked) {
					Save_Dynamic -regkey "Solutions\Autopilot\Deploy\Update" -name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -value $Temp_Queue_Select_Item -Multi

					Remove-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -Force -ErrorAction SilentlyContinue | out-null
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -name "$(Get_GPS_Location)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -value $Temp_Queue_Select_Item -Multi
				}
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "Tips_Warning_CU_Global" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions" -Name "Tips_Warning_CU_Global" -ErrorAction SilentlyContinue) {
			"True" {
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "Tips_Warning_CU" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update" -Name "Tips_Warning_CU" -ErrorAction SilentlyContinue) {
			"True" {
				$MarkShowNewTips = $False
