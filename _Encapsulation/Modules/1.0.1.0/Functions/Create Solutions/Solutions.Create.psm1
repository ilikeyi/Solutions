	...
	if ($ISO) {
		...
		if (-not $Global:AutopilotMode -xor $Global:EventQueueMode) {
			...
			if (Image_Is_Select_IAB) {
				...
				$TestFolderMountRoute = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"
...
Function Solutions_Generate_Prerequisite
{
	...
	if ($Mount) {
		if (Image_Is_Select_IAB) {
			$Script:init_To_GPS = "$($Global:Primary_Key_Image.Master)_$($Global:Primary_Key_Image.ImageFileName)"
			$Script:CopySolutionsToRoot = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"
		...
		if ((Get-Variable -Scope global -Name "SolutionsCreateUnattendPanther_$($Script:init_To_GPS)" -ErrorAction SilentlyContinue).Value) {
			if (Image_Is_Select_IAB) {
				...
				if (Test-Path -Path $TempGetFolder -PathType Container 	-ErrorAction SilentlyContinue) {
					$TempGetFolderPanther = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount\Windows\Panther"
