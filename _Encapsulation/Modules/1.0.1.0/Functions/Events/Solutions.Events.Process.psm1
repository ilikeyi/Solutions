				...
				if ($item.Expand.Count -gt 0) {
					...
					ForEach ($itemExpandNew in $item.Expand) {
						<#
							.初始化变量
						#>
						$Temp_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($itemExpandNew.ImageFileName)\Mount"
						...
						if ((Get-Variable -Scope global -Name "Mark_Is_Mount_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)").Value) {
							...
							if ((Get-Variable -Scope global -Name "Queue_Expand_Healthy_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)").Value) {
								<#
									.触发此事件强制结束
								#>
								$test_mount_Sources_Expand = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($itemExpandNew.ImageFileName)\Mount"
...
#									New-Variable -Scope global -Name "Queue_Expand_Eject_Only_Save_$($item.Main.ImageFileName)_$($itemExpandNew.ImageFileName)" -Value $False -Force
	...
	if ($IsEjectAfterSaveNot) {
		...
		$Temp_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Mount"
...
Function Event_Assign_Not_Allowed_UI
{
	...
	Write-Host "`n  $($lang.User_Interaction): $($lang.AssignNoMount): " -NoNewline -ForegroundColor Yellow
...
Function Eject_Forcibly_All
{
	...
	ForEach ($item in $Global:Image_Rule) {
		if ($item.Main.Suffix -eq "wim") {
			...
			$Eject_Main_Do_Not_Save_Path = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($item.Main.ImageFileName)\$($item.Main.ImageFileName)\Mount"
