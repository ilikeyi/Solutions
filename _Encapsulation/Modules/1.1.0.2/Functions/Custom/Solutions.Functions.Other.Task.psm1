...
Function Other_Tasks_CTD
{
	...
	if (Image_Is_Select_IAB) {
		if (Verify_Is_Current_Same) {
			...
			$Local_Regedit_File_System = Join-Path -Path $Global:Mount_To_Route -ChildPath "$($Global:Primary_Key_Image.Master).$($Global:Primary_Key_Image.MasterSuffix)\$($Global:Primary_Key_Image.ImageFileName).$($Global:Primary_Key_Image.Suffix)\Mount\$((Get-Module -Name Solutions).Author)\$($RandomGuid)"
