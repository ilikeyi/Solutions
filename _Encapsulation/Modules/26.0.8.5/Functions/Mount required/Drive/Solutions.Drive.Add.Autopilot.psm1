<#
	...
	.自动驾驶: 添加驱动
#>
Function Drive_Add_UI_Autopilot
{
	<#
		.转换架构类型
	#>
	...
	$SearchFolderRule = @(
		...
		"$(Get_MainMasterFolder)\$($Global:Primary_Key_Image.Master)\$($Global:Primary_Key_Image.ImageFileName)\Drive\Add"
	)
	...
	Function Refres_Event_Tasks_Drive_Add_UI_Autopilot
	{
		<#
			.全局
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Autopilot\Deploy\Drive" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue) {
			$GetSelectVer = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Autopilot\Deploy\Drive" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue
		...
		<#
			.当前
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue) {
			$GetSelectVer = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue
			$GetSelectVer = $GetSelectVer | Where-Object { -not ([string]::IsNullOrEmpty($_) -or [string]::IsNullOrWhiteSpace($_))} | Select-Object -Unique
	...
	Function Drive_Add_Refresh_Autopilot_Sources
	{
		$UI_Main_Error.Text = ""
		...
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Autopilot\Deploy\Drive" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue) {
			$Temp_Assign_Task_Select = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Autopilot\Deploy\Drive" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue
		}
		...
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue) {
			$Temp_Assign_Task_Select = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -ErrorAction SilentlyContinue
		}
	...
	$UI_Main           = New-Object system.Windows.Forms.Form -Property @{
		...
		Height         = 720
		Width          = 928
		Text           = "$($lang.Drive): $($lang.AddTo) [ $($lang.Autopilot), $($lang.Event_Primary_Key): $($Global:Primary_Key_Image.Uid) ]"
	...
	$UI_Main_Dashboard_Event_Global_Clear = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		LinkBehavior   = "NeverUnderline"
		add_Click      = {
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Autopilot\Deploy\Drive" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -Force -ErrorAction SilentlyContinue | out-null

			Refres_Event_Tasks_Drive_Add_UI_Autopilot
	...
	$UI_Main_Dashboard_Event_Current_Clear = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		LinkBehavior   = "NeverUnderline"
		add_Click      = {
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -Force -ErrorAction SilentlyContinue | out-null
	...
	$UI_Main_Dont_Checke_Is_File = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Checke_Is_File.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -value "False"
			}

			Drive_Add_Refresh_Autopilot_Sources
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Skip_Check_File_Add_Autopilot" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Auto_select_Folder = New-Object system.Windows.Forms.Label -Property @{
		...
		$UI_Main_Dont_Checke_Is_RulePre = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RulePre.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -value "True"
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -value "False"
				}

				Drive_Add_Refresh_Autopilot_Sources
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_Folder_Available_Add_Autopilot" -ErrorAction SilentlyContinue) {
				"True" {
		...
		$UI_Main_Dont_Checke_Is_RuleOther = New-Object System.Windows.Forms.CheckBox -Property @{
			...
			add_Click      = {
				if ($UI_Main_Dont_Checke_Is_RuleOther.Checked) {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -value "True"
				} else {
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -value "False"
				}

				Drive_Add_Refresh_Autopilot_Sources
			}
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_Folder_RuleOther_Add_Autopilot" -ErrorAction SilentlyContinue) {
				"True" {
	...
	$UI_Main_Event_Sync_To_Global = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Event_Sync_To_Global.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive" -Name "$(Get_GPS_Location)_Is_Check_SyncTo_Global_Add_Autopilot" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Save_Click = {
		...
		if ($Temp_Queue_Drive_Add_Select.Count -gt 0) {
			if ($UI_Main_Event_Sync_To_Global.Checked) {
				Save_Dynamic -regkey "Solutions\Autopilot\Deploy\Drive" -name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -value $Temp_Queue_Drive_Add_Select -Type "MultiString"

				Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -Force -ErrorAction SilentlyContinue | out-null
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -name "$(Get_GPS_Location)$(Get_Autopilot_Location);$($Global:ImageType);Add;Auto" -value $Temp_Queue_Drive_Add_Select -Type "MultiString"
			}
	...
	$Get_Temp_Select_Drive_Add_Folder = @()
	$UI_Main_Rule.Controls | ForEach-Object {
		if ($_ -is [System.Windows.Forms.CheckBox]) {
			$Get_Temp_Select_Drive_Add_Folder += $_.Tag
		}
	}

	if ($Temp_Assign_Task_Select.Count -gt 0) {
		foreach ($item in $Temp_Assign_Task_Select) {
			if ($Get_Temp_Select_Drive_Add_Folder -NotContains $item) {
...
Function Autopilot_Drive_Add_UI_Import
{
	param
	(
		$Tasks
			...
			"Auto" {
				...
				if ($New_Tasks_Assign_Auto_Schome.Count -gt 0) {
					...
					if ($Is_Valid_New_Custom_Path.count -gt 0) {
						Drive_Add_UI -Autopilot $Is_Valid_New_Custom_Path
					} else {
						Write-Host "  $($lang.NoWork)" -ForegroundColor Red
					}
				} else {
