...
Function Feature_Disable_Match_UI
{
	...
	Function Autopilot_Feature_Disable_Match_UI_Save
	{
		...
		if ($Temp_Queue_Is_Feature_Disable_Match_Select.Count -gt 0) {
			New-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_$($Global:Primary_Key_Image.Uid)" -Value $True -Force
			New-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_Custom_Select_$($Global:Primary_Key_Image.Uid)" -Value $Temp_Queue_Is_Feature_Disable_Match_Select -Force
	...
	{
		$Temp_Assign_Task_Select = (Get-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_Custom_Select_$($Global:Primary_Key_Image.Uid)" -ErrorAction SilentlyContinue).Value
	...
	$UI_Main_Event_Clear_Click = {
		New-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_$($Global:Primary_Key_Image.Uid)" -Value $False -Force
		New-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_Custom_Select_$($Global:Primary_Key_Image.Uid)" -Value @() -Force
...
Function Feature_Disable_Match_Process
{
	...
	$Temp_Queue_Is_Feature_Disable_Match_Custom_Select = (Get-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_Custom_Select_$($Global:Primary_Key_Image.Uid)" -ErrorAction SilentlyContinue).Value
