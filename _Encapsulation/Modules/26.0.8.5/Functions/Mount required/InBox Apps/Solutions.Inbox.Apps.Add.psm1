...
Function InBox_Apps_Add_UI
{
		...
		$UI_Main_Extract_Rule_Select_Sourcest.Controls | ForEach-Object {
			if ($_ -is [System.Windows.Forms.RadioButton]) {
				if ($_.Checked) {
					...
					Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)SelectGUID" -value $_.Tag
				}
			}
		}
	...
	$UI_Main_Mask_Error_Hide_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,635"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_Mask_Error_Hide = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,638"
		Text           = $lang.Hide
	...
	$UI_Main_Mask_Dependencies_DoNot_Tips = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Dependencies_DoNot_Tips.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)_IsCheckDependencies" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -name "$(Get_GPS_Location)_IsCheckDependencies" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)_IsCheckDependencies" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)_IsCheckDependencies" -ErrorAction SilentlyContinue) {
			"True" {
	...
	$UI_Main_Mask_Dependencies_Hide_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,635"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
	...
	$UI_Main_Mask_Dependencies_Hide = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,638"
		Text           = $lang.Hide
...
"@ | Out-File -FilePath "$($TempSaveTo).ps1" -Encoding UTF8 -ErrorAction SilentlyContinue
	...
	$UI_Main_Match_Apps_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,150"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\Match.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Match_Apps_Click
	}
	$UI_Main_Match_Apps = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,152"
		Text           = $lang.MatchMode
	...
	$UI_Main_Dependencies_Click = {
		if (InBox_Apps_Check_Customize -RuleNaming -SelectApps) {
			$UI_Main_Mask_Dependencies.Visible = $True
		}
	}
	$UI_Main_Dependencies_ICO = New-Object system.Windows.Forms.PictureBox -Property @{
		Location       = "620,190"
		Height         = 22
		Width          = 22
		SizeMode       = "Zoom"
		Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\..\Assets\icon\dependencies.png")
		Cursor = [System.Windows.Forms.Cursors]::Hand
		add_Click      = $UI_Main_Dependencies_Click
	}
	$UI_Main_Dependencies = New-Object system.Windows.Forms.LinkLabel -Property @{
		Height         = 30
		Width          = 255
		Location       = "646,192"
	...
	$UI_Main.controls.AddRange((
		...
		$UI_Main_Match_Apps_ICO,
		$UI_Main_Match_Apps,
		$UI_Main_Dependencies_ICO,
	...
	$UI_Main_Mask_Dependencies.controls.AddRange((
		...
		$UI_Main_Mask_Dependencies_Hide_ICO,
		$UI_Main_Mask_Dependencies_Hide
	))
	...
	$UI_Main_Mask_Error.controls.AddRange((
		...
		$UI_Main_Mask_Error_Hide_ICO,
		$UI_Main_Mask_Error_Hide
	))
	...
	<#
		.选择全局唯一规则 GUID
	#>
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)SelectGUID" -ErrorAction SilentlyContinue) {
		$GetDefaultSelectLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\InBox" -Name "$(Get_GPS_Location)SelectGUID" -ErrorAction SilentlyContinue
	} else {
...
Function InBox_Apps_Add_To_Process
{
	...
	if ($Temp_Assign_Task_InBox_Apps.Count -gt 0) {
		ForEach ($item in $Temp_Assign_Task_InBox_Apps) {
			...
			InBox_Apps_Add_Match_Process -Name $item.Name -SearchName $item.Search -AppxSource $item.InstallPacker -CertificateSource $item.Certificate -Region $item.Region -DependencyPackage $item.Depend -Arch $item.Arch -SNTasks "$($SNTasks)/$($Temp_Assign_Task_InBox_Apps.Count)"
		}
	} else {
		Write-Host "  $($lang.NoWork)" -ForegroundColor Red
	}
...
Function InBox_Apps_Add_Match_Process
{
	...
	Write-Host "  $($lang.Architecture): ".PadRight(25) -NoNewline -ForegroundColor Yellow
	...
	foreach ($Printarch in $arch) {
		if ($Printarch -Contains $NewArch) {
			Write-Host "$($Printarch) " -NoNewline -ForegroundColor Green
		} else {
			Write-Host "$($Printarch) " -NoNewline
		}
	}

	if ($Arch -Contains $NewArch) {
...
Function Inbox_Apps_Hard_Links_Optimize
{
	...
	if ($Global:Developers_Mode) {
		Write-Host "`n  $($lang.Developers_Mode_Location): 52" -ForegroundColor Green
	}
	...
	Write-Host "  " -NoNewline
	Write-Host " $($lang.Optimize_ing) " -NoNewline -BackgroundColor White -ForegroundColor Black
	try {
