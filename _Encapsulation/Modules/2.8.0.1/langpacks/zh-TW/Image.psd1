ConvertFrom-StringData -StringData @'
	...
	ImageSourcesClickShowKey        = 允許顯示選擇主鍵選單
	ImageSourcesClickShowKeyTips    = 使用者選擇映像來源後：新增右側「選擇主鍵選單」、新增「可前往」選項：掛載、儲存、彈出
	...
	Wim_Rule_Verify                 = 驗證
	Wim_Rule_Check                  = 檢查
	Wim_Rule_CheckIntegrity         = 檢查完整性
	Wim_Rule_ReadOnly               = 只讀權限
	Wim_Rule_Optimize               = 最佳化
	ResetBase                       = 深度清理
	Wim_Rule_SupportEa              = 應用具有擴展屬性的圖像
	Wim_Rule_WIMBoot                = 可啟動
	Wim_Rule_Compact                = 將作業系統檔案壓縮
	Win_Rule_Setbootable            = 將磁碟區映像標記為可啟動映像
	Win_Rule_Setbootable_Tips       = 此參數僅適用於 Windows PE 映像。\n一個 .wim 檔案中只能將一個磁碟區映像標記為可啟動。
	Destination                     = 目的地

	Wim_Append                      = 追加
	Wim_Capture                     = 捕獲
	Wim_Capture_Tips                = 將磁碟機映像擷取到新的 WIM 檔案。
	Wim_Capture_Sources             = 捕獲來源
	Wim_Capture_Sources_Tips        = 捕獲的目錄包含所有子資料夾和資料。\n您不能捕獲空目錄。目錄必須至少包含一個檔案。
	Wim_Config_File                 = 設定檔路徑（排除項）
	Wim_Config_Edit                 = 編輯
	Wim_Config_Learn                = 學習配置清單和 WimScript.ini 文件
	...
	Wim_CreatedTime                 = 創建日期
	Wim_ModifiedTime                = 上次修改時間
	Wim_FileCount                   = 文件數目
	Wim_DirectoryCount              = 目錄數目
	...
	DefenderExclude                 = Windows 防毒排除項
	DefenderFolder                  = 將此目錄新增至 Windows 防毒排除項目。
	DefenderVolume                  = 搜尋到符合的磁碟區標名後，將此磁碟新增 Windows 防毒排除項目。
	DefenderIsAdd                   = 已新增至 Windows 防毒排除項
'@