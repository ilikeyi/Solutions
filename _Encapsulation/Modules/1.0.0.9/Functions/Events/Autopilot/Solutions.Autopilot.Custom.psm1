			...
			if ($item.Main.Suffix -eq "wim") {
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						...
						Image_Select_Public_Autopilot_Add -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathExpand
					}
				}
			}
		}
	...
	Function Autopilot_Refresh_Public_Events_All
	{
		ForEach ($item in $Global:Image_Rule) {
			if ($item.Main.Suffix -eq "wim") {
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						$Tasks = Autopilot_Public_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathExpand
...
#			Write-Host "已挂载"
	...
	#			Write-Host "已挂载"
				...
				if ($NewTasksFull.IsAutoSelectIndex -eq "Auto") {
					...
					$GUI_Image_source_selection_Tips_NoAuto = New-Object system.Windows.Forms.Label -Property @{
						...
						Width          = 412
						Padding        = "30,0,0,0"
						Text           = "$($lang.MountedIndexSelect): $($NewTasksFull.IsAutoSelectIndex)"
					}
...
#					$Autopilot_Assign_Menu_Convert_Image.Enabled = $False
	...
	Function Refresh_Config_Rule_Global
	{
		...
		<#
			.映像源选择
		#>
		if (-not [string]::IsNullOrEmpty($NewTasks.IsAutoSelectIndex)) {
...
#			Refresh_Config_Rule_Sync_To_Show
	...
	Function Autopilot_Refresh_Event_All
	{
		...
		ForEach ($item in $Global:Image_Rule) {
			if ($item.Main.Suffix -eq "wim") {
				$Tasks = Autopilot_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $item.Main.ImageFileName -Uid $item.Main.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathMain
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						$Tasks = Autopilot_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathExpand
		...
		if ($Script:InBox_Apps_Rule_Select_Single.Autopilot.Prerequisite.$NewInBoxAppsFolder.ISO.Language.count -gt 0) {
			foreach ($item in $Script:InBox_Apps_Rule_Select_Single.Autopilot.Prerequisite.$NewInBoxAppsFolder.ISO.Language) {
				...
				foreach ($Verifyfile in $Init_Folder_All_File) {
					...
					if ($FileA -eq $FileB) {
						$FlagFailed = $false
						$UI_Main_Prerequisite_Language_Name.Text = $Verifyfile
