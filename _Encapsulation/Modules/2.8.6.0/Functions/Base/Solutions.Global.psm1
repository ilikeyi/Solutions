...
Function Archive
{
	...
	Write-Host " $($lang.UpdateUnpacking) " -NoNewline -BackgroundColor White -ForegroundColor Black
...
{
	$DismPath = Join-Path -Path ([System.Environment]::SystemDirectory) -ChildPath "dism.exe" -ErrorAction SilentlyContinue
	if (Test-Path -Path $DismPath -PathType leaf) {
		$dismVersion = (Get-Item -Path $DismPath).VersionInfo.ProductVersion
