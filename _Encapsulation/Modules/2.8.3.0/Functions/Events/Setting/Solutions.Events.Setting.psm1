...
Function Event_Assign_Setting
{
	...
	$GUIImageSelectAE  = New-Object System.Windows.Forms.CheckBox -Property @{
		Height         = 30
		Width          = 450
		Padding        = "16,0,0,0"
	...
	$UI_Main_OK        = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			#>
			if ($Setting) {
				Save_Dynamic -regkey "Solutions\Suggested\$($RuleName)" -name "IsMountedExpand_Select" -value $Temp_Assign_Task_Select -Type "MultiString"
				Save_Dynamic -regkey "Solutions\Suggested\$($RuleName)" -name "NotMonutedExpand_Select" -value $Global:Queue_Assign_Not_Monuted_Expand_Select -Type "MultiString"
				Save_Dynamic -regkey "Solutions\Suggested\$($RuleName)" -name "IsEvent_Select" -value $Global:Queue_Assign_Available_Select -Type "MultiString"
			} else {
				...
				if ($UI_Main_Sync_To.Enabled) {
					if ($UI_Main_Sync_To.Checked) {
						Save_Dynamic -regkey "Solutions\Suggested\$($Global:Event_Guid)" -name "IsMountedExpand_Select" -value $Temp_Assign_Task_Select -Type "MultiString"
						Save_Dynamic -regkey "Solutions\Suggested\$($Global:Event_Guid)" -name "NotMonutedExpand_Select" -value $Global:Queue_Assign_Not_Monuted_Expand_Select -Type "MultiString"
						Save_Dynamic -regkey "Solutions\Suggested\$($Global:Event_Guid)" -name "IsEvent_Select" -value $Global:Queue_Assign_Available_Select -Type "MultiString"
					}
				}
			}
	...
	if ($Setting) {
		...
		<#
			.完成后事件：关机、何时开始
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Suggested\$($RuleName)" -Name "IsEvent_Select" -ErrorAction SilentlyContinue) {
			$GetSaveLabelGUID = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions\Suggested\$($RuleName)" -Name "IsEvent_Select" -ErrorAction SilentlyContinue
				...
				if (Test-Path -Path "$($item.Main.Path)\$($item.Main.ImageFileName).$($item.Main.Suffix)" -PathType Leaf) {
					...
					if (Test-Path -Path $wimlib -PathType Leaf) {
						...
						if (-not $Silent) {
							...
							Write-Host " $($lang.Refresh) " -NoNewline -BackgroundColor White -ForegroundColor Black
						}
						...
						if ($Global:Developers_Mode) {
							Write-Host "  $($lang.Developers_Mode_Location): 29`n" -ForegroundColor Green
						}
			...
			ForEach ($Expand in $item.Expand) {
				if ($Uid -eq $Expand.Uid) {
					...
					if ($Detailed) {
						if (Test-Path -Path $test_mount_folder_Current -PathType Leaf) {
							if ((Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -ErrorAction SilentlyContinue).'ShowCommand' -eq "True") {
								Write-Host "  $($lang.Command)" -ForegroundColor Yellow
								Write-Host "  $('-' * 80)"
								Write-Host "  Get-WindowsImage -ImagePath ""$($test_mount_folder_Current)""" -ForegroundColor Green
								Write-Host "  $('-' * 80)`n"
							}
							...
							try {
								Get-WindowsImage -ImagePath $test_mount_folder_Current -ErrorAction SilentlyContinue | ForEach-Object {
									Get-WindowsImage -ImagePath $test_mount_folder_Current -index $_.ImageIndex -ErrorAction SilentlyContinue | ForEach-Object {
										$custom_array += [pscustomobject]@{
											ImageIndex       = $_.ImageIndex
											ImageName        = $_.ImageName
											ImageDescription = $_.ImageDescription
					...
					$Global:Primary_Key_Image = @{
						...
						Path           = $Expand.Path
						FullPath       = $test_mount_folder_Current
						Index          = $custom_array
					}
