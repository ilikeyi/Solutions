...
Function Feature_More_Menu
{
	...
	Write-Host " " -NoNewline
	Write-Host $lang.Backup -ForegroundColor Green
	...
	switch -Wildcard ($NewEnter)
	{
		...
		"m" {
			...
			if (Image_Is_Select_IAB) {
				...
				ToWait -wait 2
				Image_Select_Tasks_UI -Go "Feature_More_UI_Menu"
			}
		}
		"a" {
			Clear-Host
			...
			Get_Next -DevCode "MF 1"
			ToWait -wait 2
			Feature_More_Menu
		}
		"Unpack" {
			UnPack_Create_UI
			Get_Next -DevCode "MF 2"
			ToWait -wait 2
			Feature_More_Menu
		}
		"Ceup" {
...
Function Image_Get_Apps_Package
{
	...
	if (Test-Path -Path $test_mount_folder_Current -PathType Container) {
		$custom_array = @()
		try {
			Write-Host "`n  $($lang.Operable)" -ForegroundColor Green
			...
			if (Test-Path -Path $Export_To_New_Xml -PathType Leaf) {
				...
				ForEach ($empDetail in $empDetails.wim.IMAGE) {
					...
					$custom_array += [PSCustomObject]@{
						...
						WIMBoot          = $empDetail.WIMBOOT
						Architecture     = $empDetail.WINDOWS.ARCH
						Version          = $empDetail.WINDOWS.VERSION.BUILD
						SPBuild          = $empDetail.WINDOWS.VERSION.SPBUILD
						SPLevel          = $empDetail.WINDOWS.VERSION.SPLevel
						EditionId        = $empDetail.WINDOWS.EDITIONID
						InstallationType = $empDetail.WINDOWS.InstallationType
						ProductType      = $empDetail.WINDOWS.ProductType
						ProductSuite     = $empDetail.WINDOWS.ProductSuite
						SystemRoot       = $empDetail.WINDOWS.SystemRoot
						...
						ModifiedTime     = $(Get-Date $NewLastModifiTime -Format "MM/dd/yyyy HH:mm:ss tt")
						Languages        = [string]$empDetail.WINDOWS.LANGUAGES.LANGUAGE
					}
				}
