				...
				if ($WildCard.Expand.Count -gt 0) {
					ForEach ($ExpandWildcard in $WildCard.Expand) {
						...
						if ($Global:Extension_Has_been_Run -Contains $ExpandWildcard.Uid) {
							...
							Run_Expand_Assign -NewUid $ExpandWildcard.UID -NewMain $WildCard.Main -NewExpand $ExpandWildcard -MountFileName $test_mount_folder_Current
						}
					}
				} else {
					Write-Host "  $($lang.NoWork)" -ForegroundColor Red
...
Function Run_Expand_Assign
{
	...
	if (Event_Assign_Task_Verify -Mount) {
		...
		if (Verify_Is_Current_Same) {
			...
			Expand_Process_abc -NewUid $NewUid -MainUid $NewMain.ImageFileName -ExpandUid $NewExpand.ImageFileName -NewUpdatePath $NewExpand.UpdatePath -ImageFileMount $MountFileName
