				...
				if ($WildCard.Expand.Count -gt 0) {
					ForEach ($ExpandWildcard in $WildCard.Expand) {
						...
						if ($Global:Extension_Has_been_Run -Contains $ExpandWildcard.Uid) {
							...
							Run_Expand -NewUid $ExpandWildcard.UID -NewMain $WildCard.Main -NewExpand $ExpandWildcard -MountFileName $test_mount_folder_Current
						}
					}
				} else {
...
Function Expand_Process_abc_Autopilot
{
	...
	if ((Get-Variable -Scope global -Name "Queue_Is_Update_Rule_$($MainUid)_$($ExpandUid)" -ErrorAction SilentlyContinue).Value) {
		...
		$Local_Wim_Update_Folder_Sources = "$(Get_MainMasterFolder)\$($MainUid)\$($MainUid)\Update\Wimlib\$($RandomGuid)"
...
Function Run_Expand
{
	...
	if (Event_Assign_Task_Verify -Mount) {
		...
		if (Verify_Is_Current_Same) {
			...
			Expand_Process_abc_Autopilot -NewUid $NewUid -MainUid $NewMain.ImageFileName -ExpandUid $NewExpand.ImageFileName -NewUpdatePath $NewExpand.UpdatePath -ImageFileMount $MountFileName
