	...
	Function Autopilot_Refresh_Event_All
	{
		...
		ForEach ($item in $Global:Image_Rule) {
			if ($item.Main.Suffix -eq "wim") {
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						$Tasks = Autopilot_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $test_mount_folder_Current
