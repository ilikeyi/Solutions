...
Function Image_Assign_Autopilot_Master
{
			...
			if ($item.Main.Suffix -eq "wim") {
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						...
						Image_Select_Public_Autopilot_Add -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $test_mount_folder_Current
					}
				}
			}
		}
	...
	Function Autopilot_Refresh_Public_Events_All
	{
		ForEach ($item in $Global:Image_Rule) {
			if ($item.Main.Suffix -eq "wim") {
				$Tasks = Autopilot_Public_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $item.Main.ImageFileName -Uid $item.Main.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathMain
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						$Tasks = Autopilot_Public_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -ImageName $Expand.ImageFileName -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $test_mount_folder_Current
	...
	Function Autopilot_Public_Event_Assign_Task_Verify
	{
		...
		<#
			...
			.自动驾驶：添加更新
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Cumulative_updates_Add_UI_Autopilot"
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Cumulative_updates_Add_UI_Autopilot"
		}
		...
		<#
			...
			.自动驾驶：删除更新
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Update" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Del_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Cumulative_updates_Delete_UI_Autopilot"
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Del_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Cumulative_updates_Delete_UI_Autopilot"
		}
		...
		<#
			...
			.自动驾驶：添加驱动
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Drive" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Drive_Add_UI_Autopilot"
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Add_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Drive_Add_UI_Autopilot"
		}
		...
		<#
			...
			.自动驾驶：删除驱动
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy\Drive" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Del_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Drive_Delete_UI_Autopilot"
		}
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Name "$($Master)_$($ImageName)_$(Get_Autopilot_Location)_$($Global:ImageType)_Del_Auto" -ErrorAction SilentlyContinue) {
			$FlagIsWait += "Drive_Delete_UI_Autopilot"
		}
...
#			Write-Host "已挂载"
	...
	Function Image_Select_Refresh_Install_Boot_WinRE_Autopilot
	{
		...
		$UI_Main_Reselect_New_Go_To_Wrap = New-Object system.Windows.Forms.Label -Property @{
			Height         = 30
			Width          = 460
		}
		...
		$UI_Main_Select_Assign_Multitasking.controls.AddRange((
			...
			$UI_Main_Reselect_New_Go_To_Wrap
		))
...
#				Write-Host "已挂载"
	...
	Function Refresh_Autopilot_Custom_Rule
	{
		...
		<#
			.选择全局唯一规则 GUID
		#>
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectGUID" -ErrorAction SilentlyContinue) {
			$GetDefaultSelectLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectGUID" -ErrorAction SilentlyContinue
		} else {
	...
	Function Refresh_Prerequisite_Sources
	{
		...
		if ($TempSelectAraayPreRule.ExpandFolder.count -gt 0) {
			<#
				.选择自定义目录
			#>
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectDriving" -ErrorAction SilentlyContinue) {
				$GetDefaultSelectLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectDriving" -ErrorAction SilentlyContinue
			} else {
				$GetDefaultSelectLabel = ""
			}
			...
			ForEach ($item in $TempSelectAraayPreRule) {
				...
				ForEach ($itemExpand in $item.ExpandFolder) {
					...
					$CheckBox     = New-Object System.Windows.Forms.RadioButton -Property @{
						...
						Tag       = $itemExpand
						add_Click = {
							Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectDriving" -value $this.Tag -String
							Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectConfig" -value "" -String
		...
		$UI_Main_Export_Event_Custom_Menu.controls.AddRange($GUIImageSelectEventNeedMount)
			...
			if ($Autopilot.Deploy.ImageSource.Tasks.AdditionalEdition.Count -gt 0) {
				...
				foreach ($item in $Autopilot.Deploy.ImageSource.Tasks.AdditionalEdition) {
					...
					if ($Autopilot.Deploy.ImageSource.Tasks.IsAutoSelect -contains "Additional_Edition_$($item.Uid)") {
		...
		if ([string]::IsNullOrEmpty($Temp_Save_Sources)) {
			...
			#>
				...
				if ($GroupSelectAE.count -gt 0) {
					Write-Host "`n  $($lang.AdditionalEdition): $($lang.LanguageExtractAddTo)" -ForegroundColor Yellow
					Write-Host "  $('-' * 80)"
					<#
						.选择主界面附加版本菜单
					#>
					...
					if ($Autopilot.Deploy.ImageSource.Tasks.AdditionalEdition.Count -gt 0) {
						foreach ($item in $Autopilot.Deploy.ImageSource.Tasks.AdditionalEdition) {
							if ($GroupSelectAE -contains $item.Uid) {
								Image_Set_Global_Primary_Key -Uid $item.Uid -Silent -DevCode "Autopilot - 6700"
...
#			Refresh_Config_Rule_Sync_To_Show
	...
	Function Autopilot_Refresh_Event_All
	{
		...
		ForEach ($item in $Global:Image_Rule) {
			if ($item.Main.Suffix -eq "wim") {
				$Tasks = Autopilot_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -MasterSuffix $item.main.Suffix -ImageName $item.Main.ImageFileName -Suffix $item.Main.Suffix -Uid $item.Main.Uid -MainUid $item.Main.Uid -ImageFilePath $NewFileFullPathMain
				...
				if ($item.Expand.Count -gt 0) {
					ForEach ($Expand in $item.Expand) {
						$Tasks = Autopilot_Event_Assign_Task_Verify -Master $item.Main.ImageFileName -MasterSuffix $item.Main.Suffix -ImageName $Expand.ImageFileName -Suffix $Expand.Suffix -Uid $Expand.Uid -MainUid $item.Main.Uid -ImageFilePath $test_mount_folder_Current
	...
	Function Autopilot_Event_Assign_Task_Verify
	{
		...
		<#
			.生成解决方案
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_Solutions_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.更改映像语言
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_Language_Change_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.清理组件
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_Language_Components_Clean_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.本地语言体验包：标记
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_LXPs_Region_Add_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			InBox Apps，添加
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Add_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.本地语言体验（LXPs），更新
		#>
		$Temp_Queue_Is_InBox_Apps_Update = (Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Update_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value
		...
		<#
			.本地语言体验（LXPs），删除
		#>
		$Temp_LXPs_Delete = (Get-Variable -Scope global -Name "Queue_Is_LXPs_Delete_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value
		...
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_InBox_Apps_Match_Rule_Delete_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.驱动删除
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_Drive_Delete_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		<#
			.Windows 功能：禁用，匹配
		#>
		if ((Get-Variable -Scope global -Name "Queue_Is_Feature_Disable_Match_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value) {
		...
		#>
		$Temp_Functions_Before_Task = (Get-Variable -Scope global -Name "Queue_Functions_Before_Select_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value
		...
		#>
		$Temp_Functions_Rear_Task = (Get-Variable -Scope global -Name "Queue_Functions_Rear_Select_$($Master)_$($ImageName)" -ErrorAction SilentlyContinue).Value
		...
		if ($Temp_Save_Sources.Count -gt 0) {
			...
			if ($TempSelectAraayPreRule.Count -gt 0) {
				<#
					.选择配置文件
				#>
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectConfig" -ErrorAction SilentlyContinue) {
					$GetDefaultSelectLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -Name "SelectConfig" -ErrorAction SilentlyContinue
				} else {
					$GetDefaultSelectLabel = ""
				}
				...
				ForEach ($item in $TempSelectAraayPreRule) {
					...
					$CheckBox     = New-Object System.Windows.Forms.RadioButton -Property @{
						...
						Tag       = $item.FileName
						add_Click = {
							Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectConfig" -value $this.Tag -String
							Refresh_Config_Rule_Sync_To_Show
						}
					}
	...
	Function Refresh_Prerequisite_Verify
	{
		...
		Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectGUID" -value $GUID -String
			...
			foreach ($item in $Script:InBox_Apps_Rule_Select_Single.Autopilot.Prerequisite.$NewInBoxAppsFolder.ISO.Language) {
				...
				foreach ($Verifyfile in $Init_Folder_All_File) {
					...
					if ($FileA -eq $FileB) {
						...
						$UI_Main_Prerequisite_Language_Name.ForeColor = "Green"
	...
	$UI_Main_Autopilot_View_Detailed_Canel = New-Object system.Windows.Forms.Button -Property @{
	...
	$UI_Main_Public_Auto_Hide = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Public_Auto_Hide.Checked) {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "Is_Auto_Hide_Public" -value "True" -String
			} else {
				Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "Is_Auto_Hide_Public" -value "False" -String
			}
		}
	}
	...
	$UI_Main_Public_Clear_Global = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			Remove-Item -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\Autopilot\Deploy" -Force -Recurse -ErrorAction SilentlyContinue | Out-Null
	...
	$UI_Main_Public_Clear_Current = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			Remove-Item -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Update\Autopilot" -Force -Recurse -ErrorAction SilentlyContinue | Out-Null
			Remove-Item -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\ImageSources\$($Global:MainImage)\Deploy\Drive\Autopilot" -Force -Recurse -ErrorAction SilentlyContinue | Out-Null
	...
	$UI_Main_Prerequisite_Refresh_Sources = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		LinkBehavior   = "NeverUnderline"
		add_Click      = {
			Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectDriving" -value "" -String
			Save_Dynamic -regkey "Solutions\ImageSources\$($Global:MainImage)\Deploy\Autopilot" -name "SelectConfig" -value "" -String
	...
	$UI_Main_Autopilot_View_Detailed.controls.AddRange((
		...
		$UI_Main_Autopilot_View_Detailed_Canel
	))
