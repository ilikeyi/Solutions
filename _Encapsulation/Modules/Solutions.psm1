...
Function Language
{
	...
	$Global:Author = (Get-Module -Name Solutions).Author
	...
	if ($Reset) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
			$GetLanguagePrompt = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "LanguagePrompt" -ErrorAction SilentlyContinue
			if ($GetLanguagePrompt -eq "True") {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "Language" -ErrorAction SilentlyContinue) {
					$GetLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "Language" -ErrorAction SilentlyContinue
...
Function Language_Select_GUI
{
	...
	Function Language_Refresh_Search
	{
		...
		$CurrentVersion = (Get-Module -Name Solutions).Version
	...
	$UI_Main_Dont_Prompt = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Prompt.Checked) {
				Save_Dynamic -regkey "Solutions" -name "LanguagePrompt" -value "True"
			} else {
				Save_Dynamic -regkey "Solutions" -name "LanguagePrompt" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Solutions" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
				...
				if ($_ -is [System.Windows.Forms.RadioButton]) {
					if ($_.Enabled) {
						if ($_.Checked) {
							...
							Save_Dynamic -regkey "Solutions" -name "Language" -value $_.Tag
...
Function Language_Change
{
	...
	if (Test-Path "$($PSScriptRoot)\$($CurrentVersion)\langpacks\$($lang)\Lang.psd1" -PathType Leaf) {
		...
		if (Test-Path "$($PSScriptRoot)\$($CurrentVersion)\langpacks\en-US\Lang.psd1" -PathType Leaf) {
			...
			#>
			$manifest = Invoke-Expression (Get-Content "$($PSScriptRoot)\Solutions.psd1" -Raw)
			if (Test-Path "$($PSScriptRoot)\$($manifest.ModuleVersion)\langpacks\en-US\Lang.psd1" -PathType Leaf) {
				...
				write-host "  2. https://github.com/ilikeyi/solutions" -ForegroundColor Green
...
Function Modules_Import
{
	...
	<#
		.Remove all Solutions modules
		.删除所有 Solutions 模块
	#>
	Get-Module -Name Solutions* | Where-Object -Property Name -like "Solutions*" | ForEach-Object {
		Remove-Module -Name $_.Name -Force -ErrorAction Ignore
	}

	if ($Import) {
		Import-Module -Name $PSScriptRoot\..\Modules\Solutions.psd1 -Scope Global -Force | Out-Null

		<#
			.Import all *.psd1
			.导入所有 *.psd1
		#>
		Get-ChildItem -Path "$($PSScriptRoot)\$($CurrentVersion)\Functions" -Recurse -include "Solutions.*.psd1" | ForEach-Object {
