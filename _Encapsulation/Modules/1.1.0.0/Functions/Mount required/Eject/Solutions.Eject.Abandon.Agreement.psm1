...
Function Eject_Abandon_Agreement
{
	...
	Function Eject_Abandon_Agreement_UI_Save
	{
		...
		if ($CustomSelect.Count -gt 0) {
			Save_Dynamic -regkey "Solutions\RAMDisk" -name "RAMDisk_Abandon_Disks" -value $CustomSelect -Multi

			$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Success.ico")
			$UI_Main_Error.Text = "$($lang.Save): $($lang.Done)"
		} else {
			Save_Dynamic -regkey "Solutions\RAMDisk" -name "RAMDisk_Abandon_Disks" -value "" -Multi
			$UI_Main_Error_Icon.Image = [System.Drawing.Image]::Fromfile("$($PSScriptRoot)\..\..\..\Assets\icon\Error.ico")
	...
	if (-not (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name 'RAMDisk_Abandon_Disks' -ErrorAction SilentlyContinue)) {
		Save_Dynamic -regkey "Solutions\RAMDisk" -name "RAMDisk_Abandon_Disks" -value "" -Multi
	}
	$GetExcludeSoftware = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Abandon_Disks" -ErrorAction SilentlyContinue | Where-Object { -not ([string]::IsNullOrEmpty($_) -or [string]::IsNullOrWhiteSpace($_))} | Select-Object -Unique
	...
	$UI_Main_Terms_Tips_Do_Not = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Mask_Tips_Do_Not.Checked) {
				Save_Dynamic -regkey "Solutions\RAMDisk" -name "Tips_Abandon_Terms" -value "False" -String
			} else {
				Save_Dynamic -regkey "Solutions\RAMDisk" -name "Tips_Abandon_Terms" -value "True" -String
			}
		}
	}
	...
	$UI_Main_Accept    = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($This.Checked) {
				Save_Dynamic -regkey "Solutions\RAMDisk" -name "RAMDisk_Accept" -value "True" -String
				$UI_Main_Save.Enabled = $True
			} else {
				Save_Dynamic -regkey "Solutions\RAMDisk" -name "RAMDisk_Accept" -value "False" -String
				$UI_Main_Save.Enabled = $False
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Accept" -ErrorAction SilentlyContinue) {
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Accept" -ErrorAction SilentlyContinue) {
			"True" {
...
Function Refresh_Eject_Abandon_NewDisk
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Abandon_Disks" -ErrorAction SilentlyContinue) {
		$CustomRAMDISKLabel = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Abandon_Disks" -ErrorAction SilentlyContinue
			...
			"True" {
				...
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Accept" -ErrorAction SilentlyContinue) {
					switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Accept" -ErrorAction SilentlyContinue) {
						"True" {
							if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Abandon_Disks" -ErrorAction SilentlyContinue) {
								$CustomRAMDisk_Abandon_Disks = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$((Get-Module -Name Solutions).Author)\Solutions\RAMDisk" -Name "RAMDisk_Abandon_Disks" -ErrorAction SilentlyContinue
