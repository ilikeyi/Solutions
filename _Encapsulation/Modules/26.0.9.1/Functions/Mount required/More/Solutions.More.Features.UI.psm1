...
Function Feature_More_UI
{
	...
	Function Refres_Event_Tasks_Feature_More
	{
		...
		if ((Get-Variable -Scope global -Name "Queue_Is_Update_Curing_$($Global:Primary_Key_Image.Uid)" -ErrorAction SilentlyContinue).Value) {
			...
			$UI_Main_Curing_Update.ForeColor = "#008000"
			...
			$UI_Main_Curing_Update.ForeColor = "#FF0000"
	...
	if ($Autopilot) {
		...
		<#
			.固化更新
		#>
		if ($Autopilot.CuringUpdate.isAllow) {
			$UI_Main_Curing_Update.Checked = $True
		}
		...
		<#
			.固化更新 + 深度
		#>
		if ($Autopilot.CuringUpdate.ResetBase) {
			$UI_Main_Curing_Update_ResetBase.Checked = $True
		}
		...
		<#
			.固化更新 + 优化
		#>
		if ($Autopilot.CuringUpdate.Defer) {
			$UI_Main_Curing_Update_Optimize.Checked = $True
		}
