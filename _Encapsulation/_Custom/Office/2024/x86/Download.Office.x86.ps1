...
if (Test-Path $Init_Setup -PathType Leaf) {
	Write-Host "`n   $($PSScriptRoot)\Download.Office.x86.xml" -ForegroundColor Green
	if (Test-Path "$($PSScriptRoot)\Download.Office.x86.xml" -PathType Leaf) {
		start-process $Init_Setup -ArgumentList "/Download $($PSScriptRoot)\Download.Office.x86.xml" -wait -WindowStyle Hidden
	} else {
