...
if (Test-Path $Init_Setup -PathType Leaf) {
	Write-Host "`n   $($PSScriptRoot)\Config.amd64.xml" -ForegroundColor Green
	if (Test-Path "$($PSScriptRoot)\Config.amd64.xml" -PathType Leaf) {
		start-process $Init_Setup -ArgumentList "/Download $($PSScriptRoot)\Config.amd64.xml" -wait -WindowStyle Hidden
	} else {
