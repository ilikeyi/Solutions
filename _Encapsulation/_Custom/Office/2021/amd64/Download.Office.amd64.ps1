...
if (Test-Path $Init_Setup -PathType Leaf) {
	...
	$start_time = Get-Date
	Invoke-WebRequest -Uri "https://officecdn.microsoft.com/pr/wsus/setup.exe" -OutFile $Init_Setup  -TimeoutSec 30 -UseBasicParsing -DisableKeepAlive -ErrorAction SilentlyContinue | Out-Null
	Write-Host "`n   Time Used: $((Get-Date).Subtract($start_time).Seconds) (s)`n"
}

if (Test-Path $Init_Setup -PathType Leaf) {
	Write-Host "`n   $($PSScriptRoot)\Download.Office.amd64.xml" -ForegroundColor Green
	if (Test-Path "$($PSScriptRoot)\Download.Office.amd64.xml" -PathType Leaf) {
		start-process $Init_Setup -ArgumentList "/Download $($PSScriptRoot)\Download.Office.amd64.xml" -wait -WindowStyle Hidden
	} else {
