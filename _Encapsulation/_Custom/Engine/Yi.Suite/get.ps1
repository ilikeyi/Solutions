...
$AvailableLanguages = @(
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Conéctate a internet Yi's Suite"
	...
	@{
		...
		Language = @{
			FontsUI                  = "Segoe UI"
			Get                      = "Ligue-se à internet Yi's Suite"
...
Function Installation_interface_UI
{
	...
	Function Install_Init_Disk_To
	{
		switch ($UI_Main_Install_To.SelectedItem.Path) {
			"AutoSelectDisk" {}
			"Desktop" {
				Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value "Desktop"
				return Join-Path -Path $([Environment]::GetFolderPath("Desktop")) -ChildPath $Default_directory_name
			}
			"Download" {
				Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value "Download"
			...
			"Documents" {
				Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value "Documents"
			...
			default {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "Instl_To_Custom" -ErrorAction SilentlyContinue) {
					$GetNewInstallTo = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "Instl_To_Custom" -ErrorAction SilentlyContinue
					Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value $GetNewInstallTo
		...
		Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value "AutoSelectDisk"
		$ExcludeDisk = @(
	...
	$IsAllowSHA256Check = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($IsAllowSHA256Check.Checked) {
				Save_Dynamic -regkey "Suite\Get" -name "IsAllowSHA256Check" -value "True"
			} else {
				Save_Dynamic -regkey "Suite\Get" -name "IsAllowSHA256Check" -value "False"
			}
		}
	}
		...
		add_Click      = {
			...
			if ($FolderBrowser.ShowDialog() -eq "OK") {
				if (Test_Available_Disk -Path $FolderBrowser.SelectedPath) {
					...
					Save_Dynamic -regkey "Suite\Get" -name "Instl_To_Custom" -value $UI_Main_Save_To_Path.Text
					Save_Dynamic -regkey "Suite\Get" -name "InstlTo" -value $UI_Main_Save_To_Path.Text
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "InstlTo" -ErrorAction SilentlyContinue) {
		$GetNewInstallTo = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "InstlTo" -ErrorAction SilentlyContinue
	...
	#>
		...
		if ($DvHash) {
			...
			if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "IsAllowSHA256Check" -ErrorAction SilentlyContinue) {
				$GetIsAllowSHA256Check = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Name "IsAllowSHA256Check" -ErrorAction SilentlyContinue
...
#>
if ($Reset) {
	Remove-Item -Path "HKCU:\SOFTWARE\Yi\Suite\Get" -Force -Recurse -ErrorAction SilentlyContinue | Out-Null
}
