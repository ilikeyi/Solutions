...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = Join-Path -Path ([Environment]::GetFolderPath("Desktop")) -ChildPath "Upgrade.Package\YiSuite"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = Join-Path -Path $env:Temp -ChildPath "Temp.YiSuite.Upgrade.Package"
...
Function Update_Create_UI
{
	...
	$UI_Main_Create_ASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			...
			if ($UI_Main_Create_ASC.Checked) {
				...
				Save_Dynamic -regkey "Suite\GPG" -name "IsPGP" -value "True"
			} else {
				$UI_Main_Create_ASC_Panel.Enabled = $False
				Save_Dynamic -regkey "Suite\GPG" -name "IsPGP" -value "False"
			}
		}
	}
	...
	$UI_Main_Create_ASCSignName = New-Object system.Windows.Forms.LinkLabel -Property @{
		...
		add_Click      = {
			...
			if ($Newgpglistkey.Count -gt 0) {
				...
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\GPG" -Name "PGP" -ErrorAction SilentlyContinue) {
					$UI_Main_Create_ASCSign.Text = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\GPG" -Name "PGP" -ErrorAction SilentlyContinue
				}
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "Suite\GPG" -name "PGP" -value $UI_Main_Create_ASCSign.Text
	...
	if (Test-Path -Path $Verify_Install_Path -PathType leaf) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\GPG" -Name "IsPGP" -ErrorAction SilentlyContinue) {
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\GPG" -Name "IsPGP" -ErrorAction SilentlyContinue) {
				"True" {
...
@"
{
	...
	"url": "$((Get-Module -Name Engine).HelpInfoURI)/solutions/update/Yi.Suite/latest.zip"
}
