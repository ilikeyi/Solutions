...
Function Shortcut_Process
{
	...
	if ($FlagsClearSolutionsRule) {
		...
		$arguments = @(
			...
			"/i:""$($IconFolder)\Assets\icon\Engine.Gift.ico"""
		)
		...
		#region Start menu
		$NewLnkPath = "$($StartMenu)\$($Global:Author)'s Solutions.lnk"
		write-host "`n  $($lang.Filename): " -NoNewline -ForegroundColor Yellow
		Write-Host $NewLnkPath -ForegroundColor Green
		Write-Host "  $('-' * 80)"
		...
		$arguments = @(
			...
			"/i:""$($IconFolder)\Assets\icon\MainPanel.ico"""
		)
		...
		#region Main Folder
		$NewLnkPath = Join-Path -Path $(Convert-Path -Path "$($PSScriptRoot)\..\..\..\..\..\.." -ErrorAction SilentlyContinue) -ChildPath "$($Global:Author)'s Solutions.lnk"
		write-host "`n  $($lang.Filename): " -NoNewline -ForegroundColor Yellow
		Write-Host $NewLnkPath -ForegroundColor Green
		Write-Host "  $('-' * 80)"
