	...
	$GUIFEOK           = New-Object system.Windows.Forms.Button -Property @{
		...
		add_Click      = {
			...
			if ($GUIFEFixMainFolder.Checked) {
				...
				write-host "  $($lang.FixMainFolder): $($Global:Author)"
				write-host "  $($lang.Inoperable)" -ForegroundColor Red
			}
...
#		IPv6_Component -Disable               # Disable, Enabled             | $lang.IPv6Component
		...
		<#
			...
			.资源管理器
		#>
		Discover_Folder_Types -Disable  # Disable, Enabled             | $lang.DiscoverFolderTypes
...
{
	write-host "`n  $($lang.FixMainFolder): $($Global:Author)"
	...
	if (Test-Path $DeskEdit -PathType Leaf) {
		$arguments = @(
			...
			"/S=.ShellClassInfo"
			"/L=LocalizedResourceName=""$($Global:Author)'s Solutions"""
		)
		...
		$IconPath = Join-Path -Path $IconFolder -ChildPath "Assets\icon\Engine.ico"
