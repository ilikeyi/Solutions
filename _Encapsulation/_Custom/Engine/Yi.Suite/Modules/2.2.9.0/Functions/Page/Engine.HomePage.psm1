...
Function Logo
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Auto_Update_New_Version" -ErrorAction SilentlyContinue) {
		$SaveOldVersion = (Get-Module -Name Engine).Version.ToString()
		$SaveNewVersion = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Auto_Update_New_Version" -ErrorAction SilentlyContinue
		...
		"lang *" {
			...
			switch ($NewLanguage) {
				...
				"auto" {
					...
					write-host "  $('-' * 80)"
					Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "Language" -ErrorAction SilentlyContinue
				...
				default {
					...
					if (Test-Path "$($Langpacks_Sources)\$($NewLanguage)\Lang.psd1" -PathType Leaf) {
						write-host "  $($lang.Done)" -ForegroundColor Green
						Save_Dynamic -regkey "Suite" -name "Language" -value $NewLanguage
						Modules_Refresh -Function "ToMainpage -wait 2", "Mainpage"
					} else {
