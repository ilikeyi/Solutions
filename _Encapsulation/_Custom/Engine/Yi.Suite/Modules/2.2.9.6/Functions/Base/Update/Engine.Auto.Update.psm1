...
Function Auto_Update
{
	...
	Save_Dynamic -regkey "Suite\Update" -name "Last_Auto_Update_Time" -value $ticks -Type "QWord"
...
Function Auto_Update_Process
{
	...
	if ($ServerTest) {
		...
		Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateServerTestFailed
	...
	if ($error.Count -eq 0) {
		...
		Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateConnectFailed
...
$($getSerVer.changelog.log)`n"
				...
				Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_New_Version" -value $getSerVer.version.version
				If ($Force) {
					Auto_Update_And_Download -url $url -NewSHA $NewFileSha256
				} else {
					Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateNewLatest
					Write-Host "  $($lang.UpdateNewLatest)" -ForegroundColor Green
				}
			} else {
				Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateUnavailable
				Write-Host "    $($lang.UpdateUnavailable)" -ForegroundColor Red
				return
			}
		} else {
			Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value "$($lang.Auto_Update_IsLatest)"
		...
		Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $NewErrorTips
...
Function Auto_Update_And_Download
{
	...
	if ([string]::IsNullOrEmpty($NewSHA)) {
		...
		if ($localFileHash -eq $NewSHA) {
			...
			Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.Verify_Failed
			return
		}
	}
	write-host
	...
	if (Test-Path -Path $output -PathType Leaf) {
			...
			switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "IsUpdate_Clean_Allow" -ErrorAction SilentlyContinue) {
				"True"  {
					if ($SaveOldVersionShort -eq $SaveNewVersion) {
						...
						Save_Dynamic -regkey "Suite\Update" -name "IsUpdate_Clean" -value $SaveOldVersion
		...
		Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateDone
	} else {
		Save_Dynamic -regkey "Suite\Update" -name "Auto_Update_Last_status" -value $lang.UpdateUpdateStop
