...
Function Prerequisite
{
	...
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "IsUpdate_Clean" -ErrorAction SilentlyContinue) {
		$GetOldVersion = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "IsUpdate_Clean" -ErrorAction SilentlyContinue
		...
		if ($GetOldVersion -eq $SaveCurrentVersion) {
			...
			Remove-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "IsUpdate_Clean" -Force -ErrorAction SilentlyContinue | out-null
		...
		switch (Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "IsAutoUpdate" -ErrorAction SilentlyContinue) {
			"True"  {
				...
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "AutoCheckUpdate_Hours" -ErrorAction SilentlyContinue) {
					$WaitHoursTime = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "AutoCheckUpdate_Hours" -ErrorAction SilentlyContinue
				} else {
					$WaitHoursTime = "2"
				...
				<#
					.判断是否有上次执行时间
				#>
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Last_Auto_Update_Time" -ErrorAction SilentlyContinue) {
					$GetLastAutoUpdateTime = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Last_Auto_Update_Time" -ErrorAction SilentlyContinue
...
#					Write-Host "  Time: $($timeDiff.TotalSeconds) Seconds"
					...
					if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Auto_Update_Last_status" -ErrorAction SilentlyContinue) {
						$GetAuto_Update_Last_status = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite\Update" -Name "Auto_Update_Last_status" -ErrorAction SilentlyContinue
					} else {
