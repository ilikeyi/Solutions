...
Function Language
{
	...
	if ($Reset) {
		...
		if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
			$GetLanguagePrompt = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "LanguagePrompt" -ErrorAction SilentlyContinue
			if ($GetLanguagePrompt -eq "True") {
				if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "Language" -ErrorAction SilentlyContinue) {
					$GetLanguage = Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "Language" -ErrorAction SilentlyContinue
	...
	$UI_Main_Dont_Prompt = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Prompt.Checked) {
				Save_Dynamic -regkey "Suite" -name "LanguagePrompt" -value "True"
			} else {
				Save_Dynamic -regkey "Suite" -name "LanguagePrompt" -value "False"
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$($Global:Author)\Suite" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
				...
				if ($_ -is [System.Windows.Forms.RadioButton]) {
					if ($_.Enabled) {
						if ($_.Checked) {
							...
							Save_Dynamic -regkey "Suite" -name "Language" -value $_.Tag
	...
	if (Test-Path "$($PSScriptRoot)\$($CurrentVersion)\langpacks\$($lang)\Lang.psd1" -PathType Leaf) {
		...
		if (Test-Path "$($PSScriptRoot)\$($CurrentVersion)\langpacks\en-US\Lang.psd1" -PathType Leaf) {
			...
			if (Test-Path "$($PSScriptRoot)\$($manifest.ModuleVersion)\langpacks\en-US\Lang.psd1" -PathType Leaf) {
				...
				write-host "  2. https://github.com/ilikeyi/YiSuite" -ForegroundColor Green
