	...
	$UI_Main_Dont_Prompt = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Dont_Prompt.Checked) {
				Save_Dynamic -regkey "LXPs" -name "LanguagePrompt" -value "True" -String
			} else {
				Save_Dynamic -regkey "LXPs" -name "LanguagePrompt" -value "False" -String
			}
		}
	}
	if (Get-ItemProperty -Path "HKCU:\SOFTWARE\$((Get-Module -Name LXPs).Author)\LXPs" -Name "LanguagePrompt" -ErrorAction SilentlyContinue) {
				...
				if ($_ -is [System.Windows.Forms.RadioButton]) {
					if ($_.Enabled) {
						if ($_.Checked) {
							...
							Save_Dynamic -regkey "LXPs" -name "Language" -value $_.Tag -String
